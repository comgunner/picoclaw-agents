...
```bash
# 提问
docker compose run --rm picoclaw-agent -m "2+2等于几？"
...
# 预配置模板：
picoclaw onboard --openai      # 使用 OpenAI 模板 (o3-mini)
picoclaw onboard --openrouter  # 使用 OpenRouter 模板 (openrouter/auto)
...
> 运行 `picoclaw auth login --provider anthropic` 粘贴您的 API token。
...
`mcp_servers` 配置示例（请使用安装/onboard 生成的 `picoclaw` 绝对路径）：
...
当运行了多个机器人实例时会发生这种情况。请确保一次只运行一个 `picoclaw gateway`。
