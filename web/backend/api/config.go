...
func validateConfig(cfg *config.Config) []string {
	...
	if cfg.Channels.WeCom.Enabled {
		if cfg.Channels.WeCom.Token == "" {
			errs = append(errs, "channels.wecom.token is required when wecom channel is enabled")
		}
		if cfg.Channels.WeCom.EncodingAESKey == "" {
			errs = append(errs, "channels.wecom.encoding_aes_key is required when wecom channel is enabled")
		}
	}
