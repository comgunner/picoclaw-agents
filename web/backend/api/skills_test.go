...
func TestHandleGetSkill_NativeSkill(t *testing.T) {
	configPath, cleanup := setupOAuthTestEnv(t)
	defer cleanup()
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/api/skills/queue_batch", nil)
	mux.ServeHTTP(rec, req)
	...
	if resp.Source != "native" {
		t.Fatalf("resp.Source = %q, want \"native\"", resp.Source)
	}
	if resp.Content == "" {
		t.Fatalf("resp.Content is empty, want non-empty skill content")
	}
}
