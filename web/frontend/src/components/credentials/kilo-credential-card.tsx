...
interface KiloCredentialCardProps {
  status?: OAuthProviderStatus
  activeAction: string
  onAskLogout: () => void
}

export function KiloCredentialCard({
  status,
  activeAction,
  onAskLogout,
}: KiloCredentialCardProps) {
  ...
  const handleSaveToken = async () => {
    ...
    try {
      ...
      console.error('Error saving Kilo API key:', error)
    ...
    <CredentialCard
      title={
        <span className="inline-flex items-center gap-2">
          <span className="border-muted inline-flex size-6 items-center justify-center rounded-full border bg-black text-white">
            <span className="text-[10px] font-bold leading-none">K</span>
          </span>
          <span>Kilo AI</span>
        </span>
      }
      description="API key authentication for Kilo AI Gateway"
