...
2026-03-31 🎉 **ICUETH Fork Integration**: Phân tích toàn diện fork icueth hoàn tất. So sánh kiến trúc: A2A ngang (109 agents) vs subagents song song. Tính năng đã xác định: Hệ thống Họp Agent, Hệ thống Persona, bộ nhớ RAG/SQLite, hỗ trợ MCP, hệ thống Mailbox. Chiến lược tích hợp xác định 3 giai đoạn. Kiểm toán hoàn tất: ✅ CLEAN. Xem [CHANGELOG.md](CHANGELOG.md).

2026-03-31 🎉 **Qwen & Zhipu WebUI Auth**: Thêm xác thực Qwen Portal và Zhipu AI qua WebUI tại `http://localhost:18800/credentials`. CLI: `./picoclaw-agents auth login --provider qwen` và `--provider zhipu`. Tự động cấu hình models sau xác thực. Zhipu glm-4.5-flash 100% MIỄN PHÍ. Xem [CHANGELOG.md](CHANGELOG.md).

2026-03-28 🎉 **Di trú Đa-Nguồn + Team Mode Onboard**: Thêm `picoclaw-agents migrate --from nanoclaw` để di trú từ NanoClaw. Wizard onboard giờ bao gồm **Team Mode** với template dựng sẵn (Dev Team 9 agents, Research Team 3 agents, General Team 3 agents) và chọn **14 native skills**. Cải thiện Context Window: pruning tool results (-60% tokens), compact nâng cao với model override, và lệnh thủ công `/compact`. Xem [CHANGELOG.md](CHANGELOG.md).
...
2026-03-27 🎉 **WebUI Launcher hoạt động hoàn chỉnh**: `picoclaw-agents-launcher` giờ hoạt động end-to-end — nút Start Gateway, chat WebSocket qua PicoChannel, nội dung native skill trong Skills page, và tất cả section đã xác thực. Chạy với `picoclaw-agents-launcher` hoặc `picoclaw-agents-launcher -public` để truy cập mạng.

2026-03-27 🎉 **Pipeline 3 binary**: GoReleaser giờ xuất ra cả ba binary — `picoclaw-agents` (CLI), `picoclaw-agents-launcher` (WebUI), và `picoclaw-agents-launcher-tui` (TUI) — khớp cấu trúc release của dự án gốc. Trigger với `./scripts/create-release.sh`.

2026-03-26 🎉 **Tài liệu MCP Builder**: Tài liệu đầy đủ MCP Builder Agent bằng tiếng Anh và tiếng Tây Ban Nha với tham khảo API, use cases và ví dụ. Xem [docs/MCP_BUILDER_AGENT.md](docs/MCP_BUILDER_AGENT.md) và [docs/MCP_BUILDER_AGENT.es.md](docs/MCP_BUILDER_AGENT.es.md).

2026-03-26 🎉 **Lệnh Sandbox & Codegen**: Thêm `sandbox init/status` cho workspaces cô lập và `util codegen` cho tạo boilerplate Go. Xem [CHANGELOG.md](CHANGELOG.md) để biết chi tiết đầy đủ.

2026-03-26 🎉 **Auth Token Monitor**: Thêm lệnh `auth tokens` và `auth monitor` để theo dõi hết hạn token OAuth. Xem [CHANGELOG.md](CHANGELOG.md).

2026-03-26 🎉 **Config Validator & Secret Masking**: Thêm lệnh `config validate` để xác thực schema và che giấu bí mật trong wizard onboard. Xem [CHANGELOG.md](CHANGELOG.md).

2026-03-26 🎉 **Doctor Command**: Thêm lệnh `doctor` để chẩn đoán môi trường bao gồm phát hiện WSL và kiểm tra bảo mật. Xem [CHANGELOG.md](CHANGELOG.md).

2026-03-12 🎉 **Hỗ trợ Antigravity & Ổn định**: Hỗ trợ đầy đủ Google Antigravity OAuth với sanitization schema, fix deadlock TokenBudget, cải thiện rehydration session, lệnh mới `picoclaw-agents clean`, và patterns deny tăng cường. Xem [CHANGELOG.md](CHANGELOG.md) để biết chi tiết đầy đủ.

2026-03-03 🎉 **Kiến trúc Native Skills**: Giới thiệu native skills biên dịch trực tiếp vào binary (`pkg/skills/queue_batch.go`), loại bỏ phụ thuộc file `.md` bên ngoài. Tăng cường bảo mật, hiệu suất và type safety. Xem [docs/QUEUE_BATCH.en.md](docs/QUEUE_BATCH.en.md).
...
2026-03-01 🎉 **Native Skills Sentinel**: Thêm lớp bảo mật nội bộ (`skills_sentinel.go`) cung cấp bảo vệ real-time dựa trên patterns chống prompt injection và rò rỉ hệ thống.
2026-03-01 🎉 **Tăng cường Bảo mật & Ổn định**: Xử lý đóng message bus mạnh mẽ, xử lý nền WeCom App linh hoạt, và xác thực khởi tạo tăng cường cho công cụ shell.
2026-03-01 🎉 **Fail-Close Security**: Chính sách bảo mật mạnh mẽ. Công cụ thực thi lệnh giờ thực hiện xác thực chặt chẽ patterns deny trong khởi tạo.

2026-02-27 🎉 **Khôi phục Thảm họa & Task Locks**: Triển khai Task Locks nguyên tử để ngăn va chạm agent, "Boot Rehydration" để khôi phục từ sự cố đột ngột, và Context Compactor (nâng giới hạn an toàn lên 32K tokens) để loại bỏ bùng nổ ngữ cảnh trong tác vụ code dài.
