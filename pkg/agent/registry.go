...
func (r *AgentRegistry) CanSpawnSubagent(parentAgentID, targetAgentID string) bool {
	...
	if parent.Subagents == nil || parent.Subagents.AllowAgents == nil {
		// No subagents config: allow only if allow_dynamic is true
		return parent.Subagents != nil && parent.Subagents.AllowDynamic
	}
	...
	// Target not in allowlist: allow dynamic spawn if configured and target not registered
	if parent.Subagents.AllowDynamic {
		_, targetExists := r.GetAgent(targetAgentID)
		if !targetExists {
			return true
		}
	}
...
// ResetAllToolStates resets the tool state for all registered agents.
func (r *AgentRegistry) ResetAllToolStates() {
	r.mu.RLock()
	defer r.mu.RUnlock()
	for _, agent := range r.agents {
		if agent.Tools != nil {
			agent.Tools.ResetAllStates()
		}
	}
}
