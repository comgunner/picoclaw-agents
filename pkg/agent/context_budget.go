// PicoClaw-Agents - Context budget management
// Ported from picoclaw_original/pkg/agent/context_budget.go
// Adapted for fork's provider types and tokenizer.

// ============================================================================
// ⚠️  CRITICAL: Context Budget Helpers — DO NOT REMOVE
// ============================================================================
//
// These functions provide turn boundary detection and safe truncation logic
// used by ContextManager implementations (legacy + seahorse).
//
// isOverContextBudget() checks whether assembled messages + tool definitions
// would exceed the model's context window. This enables PROACTIVE compression
// BEFORE calling the LLM, rather than reacting to 402 errors after the fact.
//
// DO NOT remove or simplify these functions. They are the safety net that
// prevents the OpenRouter Free tier token overflow issue.
//
...
// Each Turn begins at a user message and extends through all subsequent
...
func isOverContextBudget(
	...
	for _, m := range messages {
		msgTokens += tokenizer.EstimateMessageTokens(m)
	}

	toolTokens := tokenizer.EstimateToolDefsTokens(toolDefs)
