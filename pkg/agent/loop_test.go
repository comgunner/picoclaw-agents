...
// TestHandleCommand_Status ensures the /status command returns a token usage
// report and is marked as handled by the agent loop.
func TestHandleCommand_Status(t *testing.T) {
	...
	if !handled {
		t.Errorf("expected /status to be handled")
	}
	if !strings.Contains(resp, "Tokens used") {
		t.Errorf("unexpected /status response: %s", resp)
	}
}

// TestHandleCommand_BundleCancel ensures the /bundle_cancel command is handled
// by the agent loop via the fast-path.
func TestHandleCommand_BundleCancel(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "agent-test-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	...
	msg := bus.InboundMessage{
		...
		SenderID: "u1",
		ChatID:   "c1",
		Content:  "/bundle_cancel id=test_id",
		Metadata: map[string]string{},
	}

	resp, handled := al.handleCommand(context.Background(), msg)
	if !handled {
		t.Errorf("expected /bundle_cancel to be handled")
	}
	// Note: It will fail because the tracker doesn't have the ID, but that proves it was handled by the tool
	if !strings.Contains(resp, "ID no encontrado") && !strings.Contains(resp, "Cancelado") {
		t.Errorf("unexpected /bundle_cancel response: %s", resp)
	}
}

// --- /restrict_to_workspace tests ---

func makeRestrictTestEnv(t *testing.T) (al *AgentLoop, cfgFile string, cleanup func()) {
	t.Helper()
	tmpDir, err := os.MkdirTemp("", "restrict-test-*")
	if err != nil {
		t.Fatalf("temp dir: %v", err)
	}

	// Write a minimal config.json with restrict_to_workspace: false
	cfgFile = filepath.Join(tmpDir, "config.json")
	initial := `{"agents":{"defaults":{"workspace":"` + tmpDir + `","restrict_to_workspace":false}}}`
	...
	return al, cfgFile, func() { os.RemoveAll(tmpDir) }
}

func TestHandleRestrictToWorkspace_Status(t *testing.T) {
	al, _, cleanup := makeRestrictTestEnv(t)
	defer cleanup()

	result := al.handleRestrictToWorkspace("status", "cli", "direct", "sess")
	if !strings.Contains(result, "Status") {
		t.Errorf("expected status report, got: %s", result)
	}
	if !strings.Contains(result, "activate") || !strings.Contains(result, "deactivate") {
		t.Errorf("expected usage hints, got: %s", result)
	}
}

func TestHandleRestrictToWorkspace_Activate(t *testing.T) {
	al, cfgFile, cleanup := makeRestrictTestEnv(t)
	defer cleanup()

	result := al.handleRestrictToWorkspace("activate", "cli", "direct", "sess")
	if !strings.Contains(result, "ACTIVATED") {
		t.Errorf("expected ACTIVATED, got: %s", result)
	}

	// Verify in-memory config updated
	if !al.cfg.Agents.Defaults.RestrictToWorkspace {
		t.Error("expected in-memory RestrictToWorkspace=true")
	}

	// Verify config file updated
	data, _ := os.ReadFile(cfgFile)
	if !strings.Contains(string(data), `"restrict_to_workspace": true`) {
		t.Errorf("expected config file updated to true, got: %s", data)
	}
}

func TestHandleRestrictToWorkspace_Deactivate(t *testing.T) {
	al, cfgFile, cleanup := makeRestrictTestEnv(t)
	defer cleanup()

	// First activate, then deactivate
	al.handleRestrictToWorkspace("activate", "cli", "direct", "sess")
	result := al.handleRestrictToWorkspace("deactivate", "cli", "direct", "sess")

	if !strings.Contains(result, "DEACTIVATED") {
		t.Errorf("expected DEACTIVATED, got: %s", result)
	}
	...
	if al.cfg.Agents.Defaults.RestrictToWorkspace {
		t.Error("expected in-memory RestrictToWorkspace=false")
	}

	data, _ := os.ReadFile(cfgFile)
	if !strings.Contains(string(data), `"restrict_to_workspace": false`) {
		t.Errorf("expected config file updated to false, got: %s", data)
	}
}

func TestHandleRestrictToWorkspace_AlreadySameValue(t *testing.T) {
	al, _, cleanup := makeRestrictTestEnv(t)
	defer cleanup()

	// Already false — try to deactivate again
	result := al.handleRestrictToWorkspace("deactivate", "cli", "direct", "sess")
...
func TestHandleRestrictToWorkspace_InvalidAction(t *testing.T) {
	al, _, cleanup := makeRestrictTestEnv(t)
	defer cleanup()

	result := al.handleRestrictToWorkspace("bad_action", "cli", "direct", "sess")
...
func TestIsUserAuthorized(t *testing.T) {
	...
	cfg := &config.Config{
		...
		Channels: config.ChannelsConfig{
			Telegram: config.TelegramConfig{AllowFrom: config.FlexibleStringSlice{"123", "456"}},
			Discord:  config.DiscordConfig{AllowFrom: config.FlexibleStringSlice{"abc"}},
		},
	}
	...
	if !al.isUserAuthorized("telegram", "123") {
		t.Error("expected 123 to be authorized on telegram")
	}
	// Unauthorized Telegram user
	if al.isUserAuthorized("telegram", "999") {
		t.Error("expected 999 to be unauthorized on telegram")
	}
	// CLI always authorized
	if !al.isUserAuthorized("cli", "any") {
		t.Error("expected cli to always be authorized")
	}
	// Empty channel always authorized
	if !al.isUserAuthorized("", "any") {
		t.Error("expected empty channel to always be authorized")
	}
	// Unknown channel unauthorized
	if al.isUserAuthorized("unknown_channel", "123") {
		t.Error("expected unknown channel to be unauthorized")
	}
}

func TestHandleCommand_RestrictToWorkspace_NoArgs(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "cmd-restrict-test-*")
	if err != nil {
		t.Fatalf("temp dir: %v", err)
	}
	defer os.RemoveAll(tmpDir)
	...
	cfg := &config.Config{
		Agents: config.AgentsConfig{
			Defaults: config.AgentDefaults{
				Workspace: tmpDir, Model: "test-model",
	...
	msg := bus.InboundMessage{Channel: "cli", ChatID: "direct", Content: "/restrict_to_workspace"}
	resp, handled := al.handleCommand(context.Background(), msg)
	if !handled {
...
func TestHandleCommand_RestrictToWorkspace_Status(t *testing.T) {
	al, _, cleanup := makeRestrictTestEnv(t)
	defer cleanup()

	msg := bus.InboundMessage{Channel: "cli", ChatID: "direct", Content: "/restrict_to_workspace status"}
	resp, handled := al.handleCommand(context.Background(), msg)
	if !handled {
		t.Error("expected command to be handled")
	}
	if !strings.Contains(resp, "Status") {
		t.Errorf("expected status report, got: %s", resp)
	}
}
