...
//
// Upstream patch adaptation: commit 6d894d6138cb89a8bc714d69b03c9a6a14cb03d7
// Adaptations:
//   - Module paths rewritten from github.com/sipeed to github.com/comgunner
//   - fileutil.WriteFileAtomic inlined as writeFileAtomic to avoid introducing
//     a new pkg/fileutil dependency in this fork. Functionally identical:
...
//
// Multi-agent note: concurrent writers for different sessions are safe
// because each session key maps to an independent mutex shard. Concurrent
// writers for the SAME session are serialized by that shared shard lock.
...
func (s *JSONLStore) readMeta(key string) (sessionMeta, error) {
	...
	if err != nil {
		log.Printf("[WARN] memory: corrupt meta for session %q, resetting: %v", key, err)
		return sessionMeta{Key: key}, nil
	}
	return meta, nil
}

// writeMeta atomically writes the metadata file using temp + fsync + rename.
...
// writeFileAtomic writes data to path atomically using temp-file + rename.
// This is the pattern from upstream's fileutil.WriteFileAtomic; inlined here
// to avoid introducing a new pkg/fileutil dependency in this fork.
func writeFileAtomic(path string, data []byte, perm os.FileMode) error {
	...
	if serr != nil {
		os.Remove(tmpName)
		return fmt.Errorf("memory: sync temp file: %w", serr)
	}
	if cerr != nil {
		os.Remove(tmpName)
	...
	if err := os.Rename(tmpName, path); err != nil {
		os.Remove(tmpName)
		return fmt.Errorf("memory: rename temp to target: %w", err)
	}
	return nil
}

// readMessages reads messages from a JSONL file, skipping the first `skip` lines.
// Corrupt lines are logged and skipped rather than causing a hard failure,
// preserving resilience in long-running multi-agent scenarios.
func readMessages(path string, skip int) ([]providers.Message, error) {
	...
	for scanner.Scan() {
		...
		if err := json.Unmarshal(line, &msg); err != nil {
			log.Printf("[WARN] memory: corrupt JSONL line %d in %q, skipping: %v", lineNum, path, err)
			continue
		}
...
// countLines counts non-empty lines in a JSONL file without unmarshaling.
...
func (s *JSONLStore) TruncateHistory(
	...
	// leaves meta.Count stale. Counting lines is cheap and TruncateHistory
	// is not a hot path, so always re-count.
...
func (s *JSONLStore) Compact(
	...
	// Write meta BEFORE rewriting the JSONL file (crash-safety).
	meta.Skip = 0
...
// Close is a no-op for the JSONL store (no persistent connections to close).
