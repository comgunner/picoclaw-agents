...
func RefreshAccessToken(cred *AuthCredential, cfg OAuthProviderConfig) (*AuthCredential, error) {
	...
	// Use the full scopes from the provider config so the refreshed token
	// retains all permissions (e.g. cloud-platform, cclog for Antigravity).
	// Falling back to basic scopes only if the config has none.
	scopes := cfg.Scopes
	if scopes == "" {
