...
func TestPicoCreateAndAddConnection_RespectsMaxConnectionsConcurrently(t *testing.T) {
	...
	for i := 0; i < goroutines; i++ {
		go func() {
			...
			if !errors.Is(err, ErrPicoTemporary) {
...
func TestPicoRemoveConnection_CleansBothIndexes(t *testing.T) {
	ch := newTestPicoChannel(t)
...
func TestPicoBroadcastToSession_TargetsOnlyRequestedSession(t *testing.T) {
	ch := newTestPicoChannel(t)

	target := &picoConn{id: "target", sessionID: "s-target"}
	target.closed.Store(true)
	ch.picoAddConnForTest(target)

	other := &picoConn{id: "other", sessionID: "s-other"}
	ch.picoAddConnForTest(other)

	err := ch.broadcastToSession("pico:s-target", newPicoMessage(picoTypeMsgCreate, map[string]any{"content": "hello"}))
	if err == nil {
