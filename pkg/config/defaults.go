...
func QwenDefaultConfig(_ bool) *Config {
	...
	cfg.Agents.Defaults.Model = "qwen-flash"
	cfg.Agents.Defaults.MaxTokens = 32768
	for i := range cfg.Agents.List {
		if cfg.Agents.List[i].ID == "project_manager" {
			cfg.Agents.List[i].Model.Primary = "qwen-flash"
		} else {
			cfg.Agents.List[i].Model.Primary = "qwen3-coder-flash"
		}
	}
