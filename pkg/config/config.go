...
type Config struct {
	...
	configPath        string                  // Internal: path to config.json on disk
}

// ConfigPath returns the path to the config.json file used to load this config.
func (c *Config) ConfigPath() string {
	return c.configPath
}

// SetConfigPath stores the path to the config.json file.
...
type SubagentsConfig struct {
	...
	AllowDynamic        bool              `json:"allow_dynamic,omitempty"`
	Model               *AgentModelConfig `json:"model,omitempty"`
	...
	AnnounceTimeoutMs   int               `json:"announce_timeout_ms,omitempty"`
}
...
type ContextManagementConfig struct {
	CompactThreshold    float64 `json:"compact_threshold"     env:"PICOCLAW_CONTEXT_COMPACT_THRESHOLD"`
	CriticalThreshold   float64 `json:"critical_threshold"    env:"PICOCLAW_CONTEXT_CRITICAL_THRESHOLD"`
...
type BinanceToolsConfig struct {
	APIKey    string `json:"api_key"    env:"PICOCLAW_TOOLS_BINANCE_API_KEY"`
	SecretKey string `json:"secret_key" env:"PICOCLAW_TOOLS_BINANCE_SECRET_KEY"`
}

type SocialMediaFacebookConfig struct {
	DefaultPageID    string `json:"default_page_id"    env:"PICOCLAW_TOOLS_SOCIAL_FACEBOOK_DEFAULT_PAGE_ID"`
	DefaultPageToken string `json:"default_page_token" env:"PICOCLAW_TOOLS_SOCIAL_FACEBOOK_DEFAULT_PAGE_TOKEN"`
	AppID            string `json:"app_id"             env:"PICOCLAW_TOOLS_SOCIAL_FACEBOOK_APP_ID"`
	AppSecret        string `json:"app_secret"         env:"PICOCLAW_TOOLS_SOCIAL_FACEBOOK_APP_SECRET"`
	UserToken        string `json:"user_token"         env:"PICOCLAW_TOOLS_SOCIAL_FACEBOOK_USER_TOKEN"`
}

type SocialMediaXConfig struct {
	APIKey            string `json:"api_key"             env:"PICOCLAW_TOOLS_SOCIAL_X_API_KEY"`
	APISecret         string `json:"api_secret"          env:"PICOCLAW_TOOLS_SOCIAL_X_API_SECRET"`
	AccessToken       string `json:"access_token"        env:"PICOCLAW_TOOLS_SOCIAL_X_ACCESS_TOKEN"`
	AccessTokenSecret string `json:"access_token_secret" env:"PICOCLAW_TOOLS_SOCIAL_X_ACCESS_TOKEN_SECRET"`
}

type SocialMediaDiscordConfig struct {
	WebhookURL string `json:"webhook_url" env:"PICOCLAW_TOOLS_SOCIAL_DISCORD_WEBHOOK_URL"`
}

type SocialMediaToolsConfig struct {
	Facebook SocialMediaFacebookConfig `json:"facebook"`
	X        SocialMediaXConfig        `json:"x"`
	Discord  SocialMediaDiscordConfig  `json:"discord"`
}

type NotionToolsConfig struct {
...
type ImageGenToolsConfig struct {
	...
	GeminiTextModelName  string `json:"gemini_text_model_name"`  // Reference to model_list
	GeminiImageModelName string `json:"gemini_image_model_name"` // Reference to model_list

	// Ideogram
	IdeogramAPIKey         string `json:"ideogram_api_key"         env:"PICOCLAW_TOOLS_IMAGE_GEN_IDEOGRAM_API_KEY"`
	IdeogramAPIURL         string `json:"ideogram_api_url"         env:"PICOCLAW_TOOLS_IMAGE_GEN_IDEOGRAM_API_URL"`
	IdeogramModelName      string `json:"ideogram_model_name"` // Reference to model_list
	IdeogramAspectRatio    string `json:"ideogram_aspect_ratio"    env:"PICOCLAW_TOOLS_IMAGE_GEN_IDEOGRAM_ASPECT_RATIO"`
	IdeogramRenderingSpeed string `json:"ideogram_rendering_speed" env:"PICOCLAW_TOOLS_IMAGE_GEN_IDEOGRAM_RENDERING_SPEED"`
	...
	// Output directory (default: ./workspace/image_gen)
	OutputDir string `json:"output_dir" env:"PICOCLAW_TOOLS_IMAGE_GEN_OUTPUT_DIR"`

	// Prompt template paths (optional, uses embedded defaults if not set)
	ImageScriptPath    string `json:"image_script_path"     env:"PICOCLAW_TOOLS_IMAGE_GEN_IMAGE_SCRIPT_PATH"`
	ImageGenScriptPath string `json:"image_gen_script_path" env:"PICOCLAW_TOOLS_IMAGE_GEN_IMAGE_GEN_SCRIPT_PATH"`
}
...
type ToolsConfig struct {
	...
	Binance     BinanceToolsConfig     `json:"binance"`
	SocialMedia SocialMediaToolsConfig `json:"social_media"`
...
// GetSensitiveValues returns a list of all sensitive credentials present in the configuration.
// Empty strings are ignored.
func (c *Config) GetSensitiveValues() []string {
	var secrets []string
	add := func(val string) {
		if val != "" {
	...
	// Providers
	add(c.Providers.Anthropic.APIKey)
	add(c.Providers.OpenAI.APIKey)
	add(c.Providers.OpenRouter.APIKey)
	add(c.Providers.Groq.APIKey)
	add(c.Providers.Zhipu.APIKey)
	add(c.Providers.VLLM.APIKey)
	add(c.Providers.Gemini.APIKey)
	add(c.Providers.Nvidia.APIKey)
	add(c.Providers.Ollama.APIKey)
	add(c.Providers.Moonshot.APIKey)
	add(c.Providers.ShengSuanYun.APIKey)
	add(c.Providers.DeepSeek.APIKey)
	add(c.Providers.Cerebras.APIKey)
	add(c.Providers.VolcEngine.APIKey)
	...
	// Channels
	add(c.Channels.Telegram.Token)
	add(c.Channels.Feishu.AppSecret)
	add(c.Channels.Feishu.EncryptKey)
	add(c.Channels.Feishu.VerificationToken)
	add(c.Channels.Discord.Token)
	add(c.Channels.QQ.AppSecret)
	add(c.Channels.DingTalk.ClientSecret)
	add(c.Channels.Slack.BotToken)
	add(c.Channels.Slack.AppToken)
	add(c.Channels.LINE.ChannelSecret)
	add(c.Channels.LINE.ChannelAccessToken)
	add(c.Channels.OneBot.AccessToken)
	add(c.Channels.WeCom.Token)
	add(c.Channels.WeCom.EncodingAESKey)
	add(c.Channels.WeComApp.CorpSecret)
	add(c.Channels.WeComApp.Token)
	add(c.Channels.WeComApp.EncodingAESKey)

	// Tools
	add(c.Tools.Web.Brave.APIKey)
	add(c.Tools.Web.Tavily.APIKey)
	add(c.Tools.Web.Perplexity.APIKey)
	add(c.Tools.Binance.APIKey)
	add(c.Tools.Binance.SecretKey)
	add(c.Tools.SocialMedia.Facebook.DefaultPageToken)
	add(c.Tools.SocialMedia.Facebook.AppSecret)
	add(c.Tools.SocialMedia.Facebook.UserToken)
	add(c.Tools.SocialMedia.X.APIKey)
	add(c.Tools.SocialMedia.X.APISecret)
	add(c.Tools.SocialMedia.X.AccessToken)
	add(c.Tools.SocialMedia.X.AccessTokenSecret)
	add(c.Tools.Notion.APIKey)
	add(c.Tools.ImageGen.GeminiAPIKey)
	add(c.Tools.ImageGen.IdeogramAPIKey)
