...
// isPathBlacklisted checks if the file being accessed contains highly sensitive data
func isPathBlacklisted(p string) bool {
	base := strings.ToLower(filepath.Base(p))
	if base == ".env" || base == "id_rsa" || base == "id_ed25519" || base == "credentials" {
		return true
	}
	if strings.HasPrefix(base, "config") && strings.HasSuffix(base, ".json") {
		return true
	}
	if strings.Contains(p, ".aws/credentials") || strings.Contains(p, ".aws\\credentials") {
		return true
	}
	return false
}
...
func validatePath(path, workspace string, restrict bool) (string, error) {
	...
	// Always resolve symlinks to enforce blacklist on the real target
	...
	if restrict {
		...
		if res, err := filepath.EvalSymlinks(absWorkspace); err == nil {
			workspaceReal = res
		}

		if err == nil { // Path exists and was resolved
...
func NewReadFileTool(workspace string, restrict bool) *ReadFileTool {
	...
	return &ReadFileTool{fs: fs, workspace: workspace, restrict: restrict}
}
...
func (t *ReadFileTool) Execute(ctx context.Context, args map[string]any) *ToolResult {
	...
	if isPathBlacklisted(absPath) {
		return ErrorResult("Access to sensitive system file blocked by security policy.")
	}

	content, err := t.fs.ReadFile(absPath)
	if err != nil {
		return ErrorResult(err.Error())
	}

	if bytes.Contains(content, []byte{0}) {
		return ErrorResult("access denied: binary files cannot be read")
	}

	// Smart truncation to avoid context explosion (Phase A implementation)
	const maxChars = 8000
	text := string(content)
	runes := []rune(text)
	if len(runes) > maxChars {
		truncated := string(
			...
			"\n... [Content Truncated: original size %d characters, showing first %d]",
			len(runes),
			maxChars,
		)
		return NewToolResult(truncated)
	}

	return NewToolResult(text)
}

type FileLockChecker interface {
	IsFileLocked(filename string) (bool, string)
}

type LockAwareTool interface {
	SetLockChecker(checker FileLockChecker)
}
...
func (t *WriteFileTool) SetLockChecker(checker FileLockChecker) {
	t.checker = checker
}
...
func NewWriteFileTool(workspace string, restrict bool) *WriteFileTool {
	...
	return &WriteFileTool{fs: fs, workspace: workspace, restrict: restrict}
}
...
func (t *WriteFileTool) Execute(ctx context.Context, args map[string]any) *ToolResult {
	...
	err = t.fs.WriteFile(absPath, []byte(contentStr))
	if err != nil {
		return ErrorResult(err.Error())
	}
...
func NewListDirTool(workspace string, restrict bool) *ListDirTool {
	...
	return &ListDirTool{fs: fs, workspace: workspace, restrict: restrict}
}
...
func (t *ListDirTool) Execute(ctx context.Context, args map[string]any) *ToolResult {
	...
	entries, err := t.fs.ReadDir(absPath)
	if err != nil {
...
func (h *hostFs) ReadFile(path string) ([]byte, error) {
	...
	content, err := io.ReadAll(io.LimitReader(f, 10*1024*1024))
	if err != nil {
...
func (r *sandboxFs) ReadFile(path string) ([]byte, error) {
	...
	err := r.execute(path, func(root *os.Root, relPath string) error {
		...
		fileContent, err := io.ReadAll(io.LimitReader(f, 10*1024*1024))
		if err != nil {
