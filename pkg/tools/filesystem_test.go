...
// TestFilesystemTool_ReadFile_Truncation verifies that files exceeding the text limit are properly truncated
func TestFilesystemTool_ReadFile_Truncation(t *testing.T) {
	tmpDir := t.TempDir()
	testFile := filepath.Join(tmpDir, "large_test.txt")

	// Generate content larger than the 8000 char threshold
	...
	// ForLLM should contain the truncation message
	if !strings.Contains(result.ForLLM, "[Content Truncated: original size 9000") {
		t.Errorf("Expected ForLLM to contain truncation notice, got: %s", result.ForLLM)
	}

	// ForLLM should have a length roughly equal to 8000 + length of the truncation message
	if len(result.ForLLM) > 8200 {
		t.Errorf("Expected ForLLM length to be truncated around 8000 chars, got: %d", len(result.ForLLM))
	}
}
