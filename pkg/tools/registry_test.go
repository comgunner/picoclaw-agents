...
type mockWorkspaceAwareTool struct {
	mockRegistryTool
	workspace string
}

func (m *mockWorkspaceAwareTool) SetWorkspacePath(workspace string) {
...
func TestToolRegistry_RegisterInjectsGlobalWorkspace(t *testing.T) {
	r := NewToolRegistryWithWorkspace("~/custom-workspace")
	tool := &mockWorkspaceAwareTool{
		mockRegistryTool: *newMockTool("workspace_aware", "workspace aware"),
	}

	r.Register(tool)

	if tool.workspace != "~/custom-workspace" {
		t.Fatalf("expected injected workspace, got %q", tool.workspace)
	}
}

func TestToolRegistry_SetGlobalWorkspace(t *testing.T) {
	r := NewToolRegistry()
	r.SetGlobalWorkspace("/tmp/picoclaw-workspace")

	tool := &mockWorkspaceAwareTool{
		mockRegistryTool: *newMockTool("workspace_aware_2", "workspace aware"),
	}
	r.Register(tool)

	if tool.workspace != "/tmp/picoclaw-workspace" {
		t.Fatalf("expected updated global workspace, got %q", tool.workspace)
	}
}
