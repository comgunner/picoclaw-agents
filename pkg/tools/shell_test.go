...
func TestShellTool_RestrictToWorkspace(t *testing.T) {
	tmpDir := t.TempDir()
	tool, _ := NewExecTool(tmpDir, false)
...
// TestShellTool_InvalidPattern verifies that invalid regex patterns cause initialization failure (fail-close)
func TestShellTool_InvalidPattern(t *testing.T) {
	cfg := &config.Config{}
	cfg.Tools.Exec.EnableDenyPatterns = true
	cfg.Tools.Exec.CustomDenyPatterns = []string{"["} // Invalid regex
...
// TestShellTool_NewExecTool_WorkingDirValidation verifies that NewExecToolWithConfig validates its working directory
func TestShellTool_NewExecTool_WorkingDirValidation(t *testing.T) {
	root := t.TempDir()

	// Test blacklisted path
	badDir := filepath.Join(root, ".env")
	err := os.MkdirAll(badDir, 0o755)
	if err != nil {
		t.Fatalf("failed to create bad dir: %v", err)
	}

	_, err = NewExecToolWithConfig(badDir, true, nil)
	if err == nil {
		t.Error("expected error for blacklisted working_dir in NewExecToolWithConfig")
	}

	// Should succeed if safe
	safeDir := filepath.Join(root, "safe_workspace")
	os.MkdirAll(safeDir, 0o755)
	_, err = NewExecToolWithConfig(safeDir, true, nil)
	if err != nil {
		t.Errorf("expected success for safe working_dir, got error: %v", err)
	}
}
