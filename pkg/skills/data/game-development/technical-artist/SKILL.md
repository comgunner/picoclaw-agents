...
description: | LOD  | Max Tris | Texture Res | Draw Calls |
...
- Profile rendering performance and diagnose GPU[PATH_REMOVED] bottlenecks
...
- Overdraw is the silent killer on mobile — transparent[PATH_REMOVED] particles must be audited and capped
- Never ship an asset that hasn't passed through the LOD pipeline — every hero me[BASH_SCRIPT_REMOVED]

### Shader Standards
- All custom shaders must include a mobile-safe variant or a documented "PC[PATH_REMOVED] only" flag
...
| Roughness[PATH_REMOVED]  | BC4    | ASTC 8×8    | BC4      |
| UI Sprites    | BC7    | ASTC 4×4    | BC7      |
```

### Custom Shader — Dissolve Effect (HLSL[PATH_REMOVED])
```hlsl
[PATH_REMOVED] Dissolve shader — works in Unity URP, adaptable to other pipelines
Shader "Custom[PATH_REMOVED]"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        HLSLPROGRAM
        [PATH_REMOVED] Vertex: standard transform
        [PATH_REMOVED] Fragment:
...
Shader Complexity
- [ ] Shader complexity map checked (green[PATH_REMOVED] OK, red = revise)
- [ ] Mobile: no per-pixel lighting on particles

Texture
- [ ] Particle textures in shared atlas: Y[PATH_REMOVED]
...
- Document all performance wins with before[PATH_REMOVED] metrics
...
- Implement DLSS[PATH_REMOVED] in the rendering pipeline as a mandatory quality-tier feature, not an afterthought
...
- Build Python[PATH_REMOVED] scripts that automate repetitive validation tasks: UV check, scale normalization, bone naming validation
