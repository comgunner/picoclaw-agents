...
description: Material Function: MFTriplanarMapping
...
- Build Niagara VFX systems with precise GPU[PATH_REMOVED] budget control
...
- Use the `Quality Switch` material node to create mobile[PATH_REMOVED] quality tiers within a single material graph
...
- Use the Niagara Scalability system to define Low[PATH_REMOVED] presets — test all three before ship
...
Modules:
  Initialize Particle:
    ...
    Color: From Surface Material parameter (dirt[PATH_REMOVED] driven by Material ID)

  Initial Velocity:
    Cone direction upward, 45° spread
    Speed: Uniform(150, 350) cm[PATH_REMOVED]

  Gravity Force: -980 cm[PATH_REMOVED]²

  Drag: 0.8 (friction to slow horizontal spread)

  Scale Color[PATH_REMOVED]:
...
High Quality (PC[PATH_REMOVED] high-end):
...
- Profile streaming behavior in World Partition — PCG load[PATH_REMOVED] must not cause hitches
...
- **Scalability first**: "We need Low[PATH_REMOVED] presets for this Niagara system before it ships"
...
- Use Niagara's Data Interface system to query physics scene data, me[BASH_SCRIPT_REMOVED]
...
- Implement recursive PCG: use the output of one graph as the input spline[PATH_REMOVED] for another
