...
description: | Grid Name         | Cell Size | Loading Range | Content Type        |
...
- **MANDATORY**: Cell size must be determined by target streaming budget — smaller cells = more granular streaming but more overhead; 64m cells for dense urban, 128m for open terrain, 256m+ for sparse desert[PATH_REMOVED]
...
- Runtime ha[BASH_SCRIPT_REMOVED]
...
Layer Stack (max 4 per blended region):
  ...
  Layer 1: Dirt[PATH_REMOVED] (replaces grass along worn paths)
...
Auto-Slope Rock Blend:
  WorldAlignedBlend node:
    ...
    Below threshold: Grass[PATH_REMOVED] gradient
...
- Target triangle count: 5000 per HLOD me[BASH_SCRIPT_REMOVED]
...
Step 6: Weighted Me[BASH_SCRIPT_REMOVED]
...
Streaming
- [ ] No hitches > 16ms during normal traversal at 8m[PATH_REMOVED] run speed
...
PCG
- [ ] Pre-baked for all areas > 1km²: Y[PATH_REMOVED]
- [ ] Streaming load[PATH_REMOVED] cost: ___ms (budget: < 2ms)
...
- Audit all shaders and materials for LWC compatibility: `tool_LWCToFloat()` functions replace direct world position sampling
...
- Monitor OFPA file count growth: large levels with thousands of actors generate thousands of files — establi[BASH_SCRIPT_REMOVED]
...
- Profile I[PATH_REMOVED] streaming latency on target storage hardware: SSDs vs. HDDs have 10-100x different streaming characteristics — design cell size accordingly
