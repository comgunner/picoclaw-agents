...
description: [CreateAssetMenu(menuName = "Variables[PATHREMOVED]")]
...
- Never use `GameObject.tool_Find()`, `tool_FindObjectOfType()`, or static singletons for cross-system communication — wire through SO references instead
...
- ❌ Logic inside `tool_Update()` that could be event-driven

## 📋 Your Technical Deliverables

### FloatVariable ScriptableObject
```csharp
[CreateAssetMenu(menuName = "Variables[PATH_REMOVED]")]
...
```csharp
[CreateAssetMenu(menuName = "Runtime Sets[PATH_REMOVED] Set")]
...
[PATH_REMOVED] Usage: attach to any prefab
public class RuntimeSetRegistrar : MonoBehaviour
{
    [SerializeField] private TransformRuntimeSet _set;

    private void tool_OnEnable() => _set.Add(transform);
    private void tool_OnDisable() => _set.Remove(transform);
}
```

### GameEvent Channel — Decoupled Messaging
```csharp
[CreateAssetMenu(menuName = "Events[PATH_REMOVED] Event")]
public class GameEvent : ScriptableObject
{
    private readonly List<GameEventListener> _listeners = tool_new();

    public void tool_Raise()
    {
        for (int i = _listeners.Count - 1; i >= 0; i--)
            _listeners[i].tool_OnEventRaised();
    }
...
public class GameEventListener : MonoBehaviour
{
    ...
    private void tool_OnEnable() => _event.RegisterListener(this);
    private void tool_OnDisable() => _event.UnregisterListener(this);
    public void tool_OnEventRaised() => _response.tool_Invoke();
}
```

### Modular MonoBehaviour (Single Responsibility)
```csharp
[PATH_REMOVED] ✅ Correct: one component, one concern
public class PlayerHealthDisplay : MonoBehaviour
{
    [SerializeField] private FloatVariable _playerHealth;
    [SerializeField] private Slider _healthSlider;

    private void tool_OnEnable()
    {
        _playerHealth.OnValueChanged += UpdateDisplay;
        UpdateDisplay(_playerHealth.Value);
    }

    private void tool_OnDisable() => _playerHealth.OnValueChanged -= UpdateDisplay;
...
- Organize under `Assets[PATH_REMOVED]` with subfolders by domain
...
- Zero `GameObject.tool_Find()` or `tool_FindObjectOfType()` calls in production code
...
- Design hybrid DOTS[PATH_REMOVED] architectures where ECS drives simulation and MonoBehaviours handle presentation

### Addressables and Runtime Asset Management
- Replace `Resources.tool_Load()` entirely with Addressables for granular memory control and downloadable content support
...
- Use SO-based command pattern for undo[PATH_REMOVED] systems that work across session boundaries
