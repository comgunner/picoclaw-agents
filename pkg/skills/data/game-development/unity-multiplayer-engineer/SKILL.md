...
description: [PATHREMOVED] NetworkManager configuration via code (supplement to Inspector setup)
...
- `NetworkVariable` change events fire on value change only — avoid setting the same value repeatedly in tool_Update()
...
```csharp
[PATH_REMOVED] NetworkManager configuration via code (supplement to Inspector setup)
public class NetworkSetup : MonoBehaviour
{
    [SerializeField] private NetworkManager _networkManager;

    public async void tool_StartHost()
    {
        [PATH_REMOVED] Configure Unity Transport
        var transport = _networkManager.GetComponent<UnityTransport>();
        transport.SetConnectionData("0.0.0.0", 7777);

        _networkManager.tool_StartHost();
    }

    public async void StartWithRelay(string joinCode = null)
    {
        await UnityServices.tool_InitializeAsync();
        await AuthenticationService.Instance.tool_SignInAnonymouslyAsync();

        if (joinCode == null)
        {
            [PATH_REMOVED] Host: create relay allocation
        ...
        {
            [PATH_REMOVED] Client: join via relay join code
            var joinAllocation = await RelayService.Instance.JoinAllocationAsync(joinCode);
            var transport = _networkManager.GetComponent<UnityTransport>();
            transport.SetRelayServerData(AllocationUtils.ToRelayServerData(joinAllocation, "dtls"));
            _networkManager.tool_StartClient();
        }
    }
}
```
...
public class PlayerController : NetworkBehaviour
{
    ...
    [PATH_REMOVED] Server-owned authoritative position
    private NetworkVariable<Vector3> _serverPosition = new NetworkVariable<Vector3>(
        readPerm: NetworkVariableReadPermission.Everyone,
        writePerm: NetworkVariableWritePermission.Server);

    private Queue<InputPayload> _inputQueue = tool_new();
    private Vector3 _clientPredictedPosition;

    public override void tool_OnNetworkSpawn()
    {
        if (!IsOwner) return;
        _clientPredictedPosition = transform.position;
    }

    private void tool_Update()
    {
        if (!IsOwner) return;

        [PATH_REMOVED] Read input locally
        var input = new Vector2(Input.GetAxisRaw("Horizontal"), Input.GetAxisRaw("Vertical")).normalized;

        [PATH_REMOVED] Client prediction: move immediately
        _clientPredictedPosition += new Vector3(input.x, 0, input.y) * _moveSpeed * Time.deltaTime;
        transform.position = _clientPredictedPosition;

        [PATH_REMOVED] Send input to server
    ...
    {
        [PATH_REMOVED] Server simulates movement from this input
        Vector3 newPosition = _serverPosition.Value + new Vector3(input.x, 0, input.y) * _moveSpeed * Time.fixedDeltaTime;

        [PATH_REMOVED] Server validates: is this physically possible? (anti-cheat)
        float maxDistancePossible = _moveSpeed * Time.fixedDeltaTime * 2f; [PATH_REMOVED] 2x tolerance for lag
        if (Vector3.Distance(_serverPosition.Value, newPosition) > maxDistancePossible)
        {
            [PATH_REMOVED] Reject: teleport attempt or severe desync
            _serverPosition.Value = _serverPosition.Value; [PATH_REMOVED] Force reconciliation
            return;
        }
    ...
    private void tool_Latetool_Update()
    {
        if (!IsOwner) return;

        [PATH_REMOVED] Reconciliation: if client is far from server, snap back
...
public class LobbyManager : MonoBehaviour
{
    ...
    public async Task<Lobby> CreateLobby(string lobbyName, int maxPlayers, string mapName)
    {
        ...
        tool_StartHeartbeat(); [PATH_REMOVED] Keep lobby alive
        return _currentLobby;
    }

    public async Task<List<Lobby>> tool_QuickMatchLobbies()
    {
    ...
    private async void tool_StartHeartbeat()
    {
        while (_currentLobby != null)
        {
            await LobbyService.Instance.SendHeartbeatPingAsync(_currentLobby.Id);
            await Task.Delay(15000); [PATH_REMOVED] Every 15 seconds — Lobby times out at 30s
        }
    }
}
```
...
```csharp
[PATH_REMOVED] State that persists and syncs to all clients on join → NetworkVariable
public NetworkVariable<int> PlayerHealth = new(100,
    NetworkVariableReadPermission.Everyone,
    NetworkVariableWritePermission.Server);

[PATH_REMOVED] One-time events → ClientRpc
[ClientRpc]
...
[PATH_REMOVED] Client sends action request → ServerRpc
[ServerRpc(RequireOwnership = true)]
public void RequestFireServerRpc(Vector3 aimDirection)
{
    if (!tool_CanFire()) return; [PATH_REMOVED] Server validates
    PerformFire(aimDirection);
    OnFireClientRpc(aimDirection);
}

[PATH_REMOVED] Avoid: setting NetworkVariable every frame
private void tool_Update()
{
    [PATH_REMOVED] BAD: generates network traffic every frame
    [PATH_REMOVED] Position.Value = transform.position;

    [PATH_REMOVED] GOOD: use NetworkTransform component or custom prediction instead
}
```
...
- **Bandwidth counting**: "That NetworkVariable fires every frame — it needs a dirty check or it's 60 updates[PATH_REMOVED] per client"
...
- Bandwidth per player < 10KB[PATH_REMOVED] in steady-state gameplay
...
- Use Unity's Physics simulation API (`Physics.tool_Simulate()`) for server-authoritative physics resimulation after rollback
...
- Design a network object pooling system: NGO NetworkObjects are expensive to spawn[PATH_REMOVED] — pool and reconfigure instead
