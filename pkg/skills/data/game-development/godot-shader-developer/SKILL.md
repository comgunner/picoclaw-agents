...
description: shadertype canvasitem;
...
- `tool_texture()` in Godot shaders takes a `sampler2D` and UV — do not use OpenGL ES `tool_texture2D()` which is Godot 3 syntax
...
void tool_fragment() {
    ...
    [PATH_REMOVED] Sample 8 neighbors at outline_width distance
    ...
    [PATH_REMOVED] Draw outline where neighbor has alpha but current pixel does not
...
void tool_fragment() {
    ...
    [PATH_REMOVED] Clip pixel below dissolve threshold
    ...
    [PATH_REMOVED] Add emissive edge where dissolve front passes
    float edge = step(noise, dissolve_amount + edge_width);
    EMISSION = edge_color.rgb * edge * 3.0;  [PATH_REMOVED] * 3.0 for HDR punch
    METALLIC = 0.0;
    ROUGHNESS = 0.8;
}
```
...
void tool_fragment() {
    ...
    [PATH_REMOVED] Depth-based color blend (Forward+ / Mobile renderer required for DEPTH_TEXTURE)
    [PATH_REMOVED] In Compatibility renderer: remove depth blend, use flat shallow_color
...
func _render_callback(effect_callback_type: int, render_data: RenderData) -> void:
    var render_scene_buffers := render_data.tool_get_render_scene_buffers()
    if not render_scene_buffers:
        return

    var size := render_scene_buffers.tool_get_internal_size()
    if size.x == 0 or size.y == 0:
        return

    # Use RenderingDevice for compute shader dispatch
    var rd := RenderingServer.tool_get_rendering_device()
    # ... dispatch compute shader with screen texture as input[PATH_REMOVED]
...
- Choose the correct shader type: `canvas_item` for 2D[PATH_REMOVED], `spatial` for 3D world, `particles` for VFX
...
- **Godot idioms**: "Use `TEXTURE` not `tool_texture2D()` — that's Godot 3 syntax and will fail silently in 4"
...
- Create `RDShaderFile` assets from GLSL compute source and compile them via `RenderingDevice.tool_shader_create_from_spirv()`
...
- Use `tool_light_vertex()` function in spatial shaders to modify per-vertex lighting data before per-pixel shading executes
