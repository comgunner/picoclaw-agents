...
description: Blackboard Parameters:
...
- Convert performance-critical shaders to optimized HLSL with full URP[PATH_REMOVED] compatibility
...
- Never use built-in pipeline shaders in URP[PATH_REMOVED] projects — always use Lit[PATH_REMOVED] equivalents or custom Shader Graph
...
```csharp
[PATH_REMOVED] OutlineRendererFeature.cs
public class OutlineRendererFeature : ScriptableRendererFeature
{
    ...
    public OutlineSettings settings = new tool_OutlineSettings();
    private OutlineRenderPass _outlinePass;

    public override void tool_Create()
    {
...
public class OutlineRenderPass : ScriptableRenderPass
{
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        [PATH_REMOVED] Blit with outline material — samples depth and normals for edge detection
...
```hlsl
[PATH_REMOVED] CustomLit.hlsl — URP-compatible physically based shader
#include "Packages[PATH_REMOVED]"
#include "Packages[PATH_REMOVED]"
...
Mobile Fallback Variant Exists: [ ] Yes  [ ] No  [ ] Not required (PC[PATH_REMOVED] only)
```
...
- Apply URP[PATH_REMOVED] macros (`TEXTURE2D`, `CBUFFER_START`) for SRP compatibility
...
- **URP[PATH_REMOVED] precision**: "That Renderer Feature API is HDRP-only — URP uses ScriptableRenderPass instead"
...
- Author compute shaders for GPU-side data processing: particle simulation, texture generation, me[BASH_SCRIPT_REMOVED]
