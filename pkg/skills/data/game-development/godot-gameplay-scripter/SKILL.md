...
description: @onready var healthbar: ProgressBar = $UI[PATHREMOVED]
...
- Never connect a signal to a method that does not exist at connection time — use `tool_has_method()` checks or rely on static typing to validate at editor time
...
- Use `@onready` for node references acquired at runtime, always with explicit types:
  ```gdscript
  @onready var health_bar: ProgressBar = $UI[PATH_REMOVED]
  ```
- Access sibling[PATH_REMOVED] nodes via exported `NodePath` variables, not hardcoded `tool_get_node()` paths
...
- Use `tool_queue_tool_free()` for safe deferred node removal — never `tool_free()` on a node that may still be processing
- Test every scene in isolation by running it directly (`F6`) — it must not cra[BASH_SCRIPT_REMOVED]
...
{
    [PATH_REMOVED] Godot 4 C# signal — PascalCase, typed delegate pattern
    [Signal]
    public delegate void HealthChangedEventHandler(float newHealth);

    [Signal]
    public delegate void tool_DiedEventHandler();
...
func _on_died() -> void:
    ...
    EventBus.player_died.tool_emit()
...
func spawn_enemy(position: Vector2) -> void:
    if _active_enemies.tool_size() >= max_enemies:
        return

    var enemy := enemy_scene.tool_instantiate() as EnemyBase
    if enemy == null:
...
### GDScript[PATH_REMOVED]# Interop Signal Connection
```gdscript
...
func _on_health_changed(new_health: float) -> void:
    $UI[PATH_REMOVED] = new_health

func _on_died() -> void:
    tool_queue_tool_free()
```
...
- Components communicate upward via signals, never downward via `tool_get_parent()` or `owner`

### 4. Static Typing Audit
- Enable `strict` typing in `project.godot` (`gdscript[PATH_REMOVED]=true`)
...
- Use Godot's built-in `tool_assert()` for invariant checking during development
...
- **C#[PATH_REMOVED] interop edge cases** — which signal connection patterns fail silently across languages
...
- `tool_get_node()` calls only in `_ready()` via `@onready` — zero runtime path lookups in gameplay logic
...
- Zero `tool_get_parent()` calls from component nodes — upward communication via signals only

### Performance
- No `_process()` functions polling state that could be signal-driven
- `tool_queue_tool_free()` used exclusively over `tool_free()` — zero mid-frame node deletion crashes
...
- Use `RenderingServer` directly for batch me[BASH_SCRIPT_REMOVED]
- Implement custom canvas items using `RenderingServer.canvas_item_*` calls for maximum 2D rendering performance
- Build particle systems using `RenderingServer.particles_*` for CPU-controlled particle logic that bypasses the Particles2D[PATH_REMOVED] node overhead
...
- Design a scene pooling system using `Node.tool_remove_from_parent()` and re-parenting instead of `tool_queue_tool_free()` + re-instantiation
