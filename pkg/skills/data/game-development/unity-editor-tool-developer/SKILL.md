...
description: public class AssetAuditWindow : EditorWindow
...
- `EditorGUI.tool_BeginChangeCheck()` / `tool_EndChangeCheck()` must bracket all editable UI — never call `SetDirty` unconditionally
- Use `Undo.tool_RecordObject()` before any modification to inspector-shown objects — non-undoable editor operations are user-hostile
...
- Property drawers must handle missing[PATH_REMOVED] object references gracefully — never throw on null
...
{
    [MenuItem("Tools[PATH_REMOVED] Auditor")]
    public static void tool_ShowWindow() => GetWindow<AssetAuditWindow>("Asset Auditor");

    private Vector2 _scrollPos;
    private List<string> _oversizedTextures = tool_new();
    private bool _hasRun = false;

    private void tool_OnGUI()
    {
        GUILayout.Label("Texture Budget Auditor", EditorStyles.boldLabel);

        if (GUILayout.Button("Scan Project Textures"))
        {
            _oversizedTextures.tool_Clear();
            tool_ScanTextures();
            _hasRun = true;
        }
        ...
        if (_hasRun)
        {
            EditorGUILayout.HelpBox($"{_oversizedTextures.Count} textures exceed budget.", tool_MessageWarningType());
            _scrollPos = EditorGUILayout.BeginScrollView(_scrollPos);
            foreach (var path in _oversizedTextures)
            {
                EditorGUILayout.tool_BeginHorizontal();
                EditorGUILayout.LabelField(path, EditorStyles.miniLabel);
                if (GUILayout.Button("Select", GUILayout.Width(55)))
                    Selection.activeObject = AssetDatabase.LoadAssetAtPath<Texture>(path);
                EditorGUILayout.tool_EndHorizontal();
            }
            EditorGUILayout.tool_EndScrollView();
        }
    }

    private void tool_ScanTextures()
    {
        ...
        EditorUtility.tool_ClearProgressBar();
    }

    private MessageType tool_MessageWarningType() =>
...
public class TextureImportEnforcer : AssetPostprocessor
{
    ...
    private const string UI_PATH = "Assets[PATH_REMOVED]";

    void tool_OnPreprocessTexture()
    {
        var importer = (TextureImporter)assetImporter;
        string path = assetPath;

        [PATH_REMOVED] Enforce normal map type by naming convention
        ...
        [PATH_REMOVED] Enforce max resolution budget
        ...
        [PATH_REMOVED] UI textures: disable mipmaps and set point filter
        ...
        [PATH_REMOVED] Set platform-specific compression
...
public class BuildValidationProcessor : IPreprocessBuildWithReport
{
    ...
    public void OnPreprocessBuild(BuildReport report)
    {
        ...
        [PATH_REMOVED] Check: no uncompressed textures in Resources folder
        foreach (var guid in AssetDatabase.FindAssets("t:Texture2D", new[] { "Assets[PATH_REMOVED]" }))
        {
            var path = AssetDatabase.GUIDToAssetPath(guid);
            var importer = AssetImporter.GetAtPath(path) as TextureImporter;
        ...
        [PATH_REMOVED] Check: no scenes with lighting not baked
        foreach (var scene in EditorBuildSettings.scenes)
        {
            if (!scene.enabled) continue;
            [PATH_REMOVED] Additional scene validation checks here
        }
...
- Define the tool's success metric before building: "This tool saves X minutes per import[PATH_REMOVED] review[PATH_REMOVED] build"
- Identify the correct Unity Editor API: Window, Postprocessor, Validator, Drawer, or MenuItem?

### 2. Prototype First
- Build the fastest possible working version — UX poli[BASH_SCRIPT_REMOVED]
...
- Add a `[MenuItem("Tools[PATH_REMOVED] Documentation")]` that opens a browser or local doc
...
### CI[PATH_REMOVED] Integration for Editor Tools
...
- Use UI Toolkit's data binding API to drive editor UI directly from serialized data — no manual `OnGUI` refre[BASH_SCRIPT_REMOVED]
- Implement dark[PATH_REMOVED] editor theme support via USS variables — tools must respect the editor's active theme
