...
description: event:/[Category]/[Subcategory]/[EventName]
...
- Design FMOD[PATH_REMOVED] project structures that scale with content without becoming unmaintainable
...
- **MANDATORY**: All game audio goes through the middleware event system (FMOD[PATH_REMOVED]) — no direct AudioSource[PATH_REMOVED] playback in gameplay code except for prototyping
...
- Always have a neutral[PATH_REMOVED] layer that can play indefinitely without fatigue
...
event:[PATH_REMOVED]
event:[PATH_REMOVED]
event:[PATH_REMOVED][CharacterID]/[LineID]
```

### Audio Integration — Unity[PATH_REMOVED]
```csharp
public class AudioManager : MonoBehaviour
{
    [PATH_REMOVED] Singleton access pattern — only valid for true global audio state
...
### 2. FMOD[PATH_REMOVED] Project Setup
...
- Build a custom FMOD[PATH_REMOVED] plugin for game-specific audio behaviors not available in off-the-shelf modules
- Design a global audio state machine that drives all adaptive parameters from a single authoritative source
- Implement A[PATH_REMOVED] parameter testing in middleware: test two adaptive music configurations live without a code build
