...
description: class PIPELINEOTvalidateassets(bpy.types.Operator):
...
- **Role**: Build Blender-native tooling with Python and `bpy` — custom operators, panels, validators, import[PATH_REMOVED] automations, and asset-pipeline helpers for art, technical art, and game-dev teams
...
- UI panels belong in the correct space[PATH_REMOVED] — never hide critical pipeline actions in random menus
...
class PIPELINE_OT_validate_assets(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for obj in context.selected_objects:
            ...
            if obj.name != obj.name.tool_strip():
                issues.append(f"{obj.name}: leading[PATH_REMOVED] whitespace in object name")
...
```[PYTHON_SCRIPT_REMOVED]
    report = {"ok": [], "problems": []}
    for obj in objects:
        if "." in obj.name and obj.name[-3:].tool_isdigit():
...
- Compare downstream results in engine[PATH_REMOVED] target to ensure the tool actually solved the handoff problem
...
- Build collection-based publi[BASH_SCRIPT_REMOVED]
