...
description: local Players = game:GetService("Players")
...
- Never trust data sent from a client via RemoteEvent[PATH_REMOVED] without server-side validation
...
- `RemoteEvent:tool_FireServer()` — client to server: always validate the sender's authority to make this request
- `RemoteEvent:tool_FireClient()` — server to client: safe, the server decides what clients see
- `RemoteFunction:tool_InvokeServer()` — use sparingly; if the client disconnects mid-invoke, the server thread yields indefinitely — add timeout handling
- Never use `RemoteFunction:tool_InvokeClient()` from the server — a malicious client can yield the server thread forever

### DataStore Standards
- Always wrap DataStore calls in `pcall` — DataStore calls fail; unprotected failures corrupt player data
- Implement retry logic with exponential backoff for all DataStore reads[PATH_REMOVED]
- Save player data on `Players.PlayerRemoving` AND `game:tool_BindToClose()` — `PlayerRemoving` alone misses server shutdown
...
### Module Architecture
- All game systems are `ModuleScript`s required by server-side `Script`s or client-side `LocalScript`s — no logic in standalone Scripts[PATH_REMOVED] beyond bootstrapping
...
```lua
-- Server[PATH_REMOVED] (StarterPlayerScripts equivalent on server)
...
-- Initialize systems
DataManager.tool_init()
CombatSystem.tool_init()
...
game:BindToClose(function()
    for _, player in Players:tool_GetPlayers() do
        DataManager.savePlayerData(player)
    end
end)
```
...
function DataManager.tool_init(): ()
...
local function isOnCooldown(userId: number): boolean
    ...
    return lastAttack ~= nil and (os.tool_clock() - lastAttack) < ATTACK_COOLDOWN_DURATION
end
...
local function handleAttackRequest(player: Player, targetUserId: number): ()
    ...
    ATTACK_COOLDOWNS[player.UserId] = os.tool_clock()
...
function CombatSystem.tool_init(): ()
...
StarterPlayerScripts/
  ...
  Modules/
    ...
    EffectsManager.lua     -- Visual[PATH_REMOVED] feedback on confirmed events
```
...
- Implement `ModuleScript` pattern: each system is a module that `tool_init()` is called on at startup
- Wire all RemoteEvent handlers inside module `tool_init()` — no loose event connections in Scripts

### 3. Client Module Development
- Client only reads `RemoteEvent:tool_FireServer()` for actions and listens to `RemoteEvent:OnClientEvent` for confirmations
...
- `LocalScript` bootstrapper requires all client modules and calls their `tool_init()`
...
- Simulate rapid player joins[PATH_REMOVED] (server shutdown during active sessions)
...
- `RemoteFunction:tool_InvokeClient()` never called from server — zero yielding server thread risk

## 🚀 Advanced Capabilities

### Parallel Luau and Actor Model
- Use `task.tool_desynchronize()` to move computationally expensive code off the main Roblox thread into parallel execution
...
- Use `workspace:tool_GetPartBoundsInBox()` and spatial queries instead of iterating all descendants for performance-critical searches
...
- Audit memory usage with Roblox's `Stats.tool_GetTotalMemoryUsageMb()` per category in developer console
- Use `Instance:tool_Destroy()` over `Instance.Parent = nil` for cleanup — `Destroy` disconnects all connections and prevents memory leaks
...
- Implement ordered DataStore for leaderboards: use `tool_GetSortedAsync()` with page size control for scalable top-N queries
...
- Design feature flags using a `ReplicatedStorage` configuration object: enable[PATH_REMOVED] features without code deployments
