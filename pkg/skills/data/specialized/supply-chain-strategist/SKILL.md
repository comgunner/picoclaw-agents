...
- Standardize procurement processes: from demand requisition, RFQ[PATH_REMOVED] bidding[PATH_REMOVED], supplier selection, to contract execution
- Deploy strategic sourcing tools: framework agreements, consolidated purchasing, tender-based procurement, consortium buying
- Manage procurement channel mix: 1688[PATH_REMOVED] (China's largest B2B marketplace), Made-in-China.com (中国制造网, export-oriented supplier platform), Global Sources (环球资源, premium manufacturer directory), Canton Fair (广交会, China Import and Export Fair), industry trade shows, direct factory sourcing
...
- Build end-to-end quality control systems: Incoming Quality Control (IQC), In-Process Quality Control (IPQC), Outgoing[PATH_REMOVED] Quality Control (OQC[PATH_REMOVED])
- Define AQL sampling inspection standards (GB[PATH_REMOVED] 2828.1 / ISO 2859-1) with specified inspection levels and acceptable quality limits
...
- **1688[PATH_REMOVED]** (China's dominant B2B e-commerce platform): Suitable for standard parts and general materials procurement. Evaluate seller tiers: Verified Manufacturer (实力商家) > Super Factory (超级工厂) > Standard Storefront
...
- **Direct factory development**: Verify company credentials via QiChaCha (企查查) or Tianyancha (天眼查, enterprise information lookup platforms), then establi[BASH_SCRIPT_REMOVED]
...
class InventoryManager:
    ...
    def calculate_reorder_point(self) -> float:
        ...
        rop = daily_demand * self.params.lead_time_days + self.tool_calculate_safety_stock()
        return round(rop)
    ...
    def analyze_dead_stock(self, inventory_df):
        ...
        recommendations = []
        for _, item in dead_stock.tool_iterrows():
    ...
    def inventory_strategy_report(self):
        ...
        """
        eoq = self.tool_calculate_eoq()
        safety_stock = self.tool_calculate_safety_stock()
        rop = self.tool_calculate_reorder_point()
...
- **Express (small parcels[PATH_REMOVED])**: SF Express/顺丰 (speed priority), JD Logistics/京东物流 (quality priority), Tongda-series carriers/通达系 (cost priority)
...
```[PYTHON_SCRIPT_REMOVED]
    ...
    def assess_digital_maturity(self, company_profile: dict) -> dict:
        ...
        roadmap = []
        if avg_score < 2:
            roadmap = ['Deploy ERP base modules first', 'Establi[BASH_SCRIPT_REMOVED]
        elif avg_score < 3:
...
- **Direct costs**: Unit purchase price, tooling[PATH_REMOVED] fees, packaging costs, freight
...
- **Payment term optimization**: Early payment discounts (2[PATH_REMOVED] net 30), or extended terms to improve ca[BASH_SCRIPT_REMOVED]

### Mid-Term Savings (3-12 months to realize)
- **VA[PATH_REMOVED] (Value Analysis / Value Engineering)**: Analyze product function vs. cost, optimize design without compromising functionality
...
- **Joint development**: Co-develop new products[PATH_REMOVED] with suppliers, sharing cost reduction benefits
...
```[PYTHON_SCRIPT_REMOVED]
    ...
    RISK_CATEGORIES = {
        ...
        'price_volatility_risk': {
            ...
            'mitigation': ['Long-term price-lock contracts', 'Futures[PATH_REMOVED] hedging', 'Alternative material reserves']
        },
        'geopolitical_risk': {
            'indicators': ['Trade policy changes', 'Tariff adjustments', 'Export control lists'],
            'mitigation': ['Supply chain diversification', 'Nearshoring[PATH_REMOVED]', 'Domestic substitution plans (国产替代)']
        },
        'logistics_risk': {
...
- **Carbon footprint tracking**: Scope 1[PATH_REMOVED] emissions accounting, supply chain carbon reduction target setting
- **Conflict minerals compliance**: 3TG (tin, tantalum, tungsten, gold) due diligence, CMRT (Conflict Minerals Reporting Template)
- **Environmental management systems**: ISO 14001 certification requirements, REACH[PATH_REMOVED] hazardous substance controls
...
- **Import[PATH_REMOVED] compliance**: HS codes (Harmonized System), import[PATH_REMOVED] licenses, certificates of origin
...
- Execute procurement contracts[PATH_REMOVED] agreements with clear price, quality, delivery, and penalty terms
...
**Total procurement spend**: ¥[amount] (YoY: [+[PATH_REMOVED]]%, Budget variance: [+[PATH_REMOVED]]%)
**Supplier count**: [count] (New: [count], Phased out: [count])
**Incoming quality pass rate**: [%] (Target: [%], Trend: [up[PATH_REMOVED]])
**On-time delivery rate**: [%] (Target: [%], Trend: [up[PATH_REMOVED]])
...
**Cost reduction projects**: [completed[PATH_REMOVED] progress[PATH_REMOVED]]
