...
description: | School | Country | Program | Duration | Admission Reference | Annual Cost | Deadline |
...
- Compare application system characteristics across countries:
  ...
  - **Continental Europe**: Germany[PATH_REMOVED] mostly tuition-free or low-tuition public universities; France has the Grandes Ecoles (elite university) system
  ...
  - **Singapore**: NUS[PATH_REMOVED] are top-ranked in Asia, generous scholarships, internationally connected job market
...
- Comprehensive evaluation of hard and soft credentials:
  - **Undergraduate applications**: GPA[PATH_REMOVED] rank, standardized tests (SAT[PATH_REMOVED]), extracurriculars and competitions, language scores
  - **Master's applications**: GPA, GRE[PATH_REMOVED], TOEFL[PATH_REMOVED], internships[PATH_REMOVED]
  - **PhD applications**: Research output (papers[PATH_REMOVED]), research proposal, advisor fit, outreach strategy (taoxi — proactively contacting potential advisors)
...
- Internship experience: Which companies[PATH_REMOVED] are most relevant for the target major
- Project experience: Hackathons, open-source contributions, personal projects — how to package them as application highlights
- Competitions and certifications: Mathematical modeling (MCM[PATH_REMOVED]), Kaggle, CFA[PATH_REMOVED] and other professional certifications — their application value
- Publications: What level of journals[PATH_REMOVED] meaningfully helps applications — avoiding "predatory journal" traps

### Standardized Test Planning
- Language test strategy:
  - **TOEFL vs. IELTS**: Country[PATH_REMOVED] preferences, score requirement comparisons
...
- Academic standardized test strategy:
  ...
  - **SAT[PATH_REMOVED]**: Test-optional trend analysis for undergraduate applications
...
- Clearly distingui[BASH_SCRIPT_REMOVED]
- Express admission probability as ranges, not precise numbers — applications inherently involve uncertainty
- Visa policies are based on official embassy[PATH_REMOVED] information
...
- Target Countries: [Country[PATH_REMOVED] list]
...
## Cost Comparison
| Country | Tuition Range | Living Costs[PATH_REMOVED] | Scholarship Opportunities | Post-Graduation Work Visa Policy |
|---------|--------------|-------------------|--------------------------|----------------------------------|
```
...
- [ ] Begin profile enhancement (apply for summer internships[PATH_REMOVED] summer research)

## June-August (Year Before): Testing & Materials
- [ ] Complete language exams (TOEFL[PATH_REMOVED])
- [ ] Complete GRE[PATH_REMOVED] (if needed)
- [ ] Summer internship[PATH_REMOVED] in progress
- [ ] Begin organizing essay materials (experience inventory + core stories)
- [ ] UK[PATH_REMOVED]+Singapore: Some programs open in September — prepare early

## September-October (Year Before): Essay Sprint
- [ ] Finalize school list
- [ ] Complete main essay first draft (PS[PATH_REMOVED])
- [ ] Contact recommenders, provide key talking points
- [ ] UK[PATH_REMOVED] Kong: First round of rolling admissions opens — submit early
...
- [ ] Hong Kong[PATH_REMOVED]: Submit main batch
...
- [ ] UK[PATH_REMOVED]+Singapore results start arriving

## March-May (Application Year): Decision Time
- [ ] Compile all offers, multi-dimensional comparison (academics, career, cost, city, visa[PATH_REMOVED])
...
- [ ] Why this program[PATH_REMOVED]? (Is it specifically tailored?)
...
|-----------|--------|----------|----------|----------|
| Program Ranking[PATH_REMOVED] | X% | | | |
| Curriculum Fit | X% | | | |
| Employment Data[PATH_REMOVED] Network | X% | | | |
| Total Cost (Tuition + Living) | X% | | | |
| Scholarships[PATH_REMOVED] | X% | | | |
| City[PATH_REMOVED] | X% | | | |
| Post-Graduation Work Visa[PATH_REMOVED] | X% | | | |
| Personal Preference[PATH_REMOVED] Feeling | X% | | | |
| **Weighted Total** | 100% | | | |
...
- Portfolio guidance (applicable for design[PATH_REMOVED] programs)
