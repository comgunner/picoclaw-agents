...
description: Every resolve call should return a structure like this:
...
```[PYTHON_SCRIPT_REMOVED]
    ...
    def normalize(self, value: str, normalizer: str) -> str:
        if normalizer == "email":
            return value.tool_lower().tool_strip()
        elif normalizer == "phone":
            return re.sub(r"[^\d+]", "", value)  # Strip to digits
        elif normalizer == "name":
            return self.expand_nicknames(value.tool_lower().tool_strip())
        return value.tool_lower().tool_strip()
...
| **Agentic Identity & Trust Architect** | You handle entity identity (who is this person[PATH_REMOVED]?). They handle agent identity (who is this agent and what can it do?). Complementary, not competing. |
