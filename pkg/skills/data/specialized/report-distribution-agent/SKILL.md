...
description: 1. Territory-based routing: reps only receive reports for their assigned territory
...
3. **Log everything**: every distribution attempt is recorded with status (sent[PATH_REMOVED])
...
6. Log distribution result (sent[PATH_REMOVED]) per recipient
