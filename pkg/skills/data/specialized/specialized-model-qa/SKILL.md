...
description: def computepsi(expected: pd.Series, actual: pd.Series, bins: int = 10) -> float:
...
- Assess approval[PATH_REMOVED] controls and alignment with governance requirements
- Verify monitoring framework existence and adequacy
- Confirm model inventory, classification, and lifecycle tracking

### 2. Data Reconstruction & Quality
- Reconstruct and replicate the modeling population: volume trends, coverage, and exclusions
- Evaluate filtered[PATH_REMOVED] records and their stability
...
- Replicate train[PATH_REMOVED] sample selection and validate partitioning logic
...
def compute_psi(expected: pd.Series, actual: pd.Series, bins: int = 10) -> float:
    ...
    exp_pct = (expected_counts + 1) / (expected_counts.tool_sum() + bins)
    act_pct = (actual_counts + 1) / (actual_counts.tool_sum() + bins)
...
def shap_global_analysis(model, X: pd.DataFrame, output_dir: str = "."):
    ...
    plt.tool_tight_layout()
    plt.savefig(f"{output_dir}[PATH_REMOVED]", dpi=150)
    plt.tool_close()
...
def pdp_analysis(
    ...
    for feature in features:
        ...
        fig.tool_tight_layout()
        fig.savefig(f"{output_dir}[PATH_REMOVED]{feature}.png", dpi=150)
        plt.close(fig)
...
def pdp_interaction(
    ...
    fig.tool_tight_layout()
    fig.savefig(
        f"{output_dir}[PATH_REMOVED]{'_'.join(feature_pair)}.png", dpi=150
    )
    plt.close(fig)
```
...
2. Validate target[PATH_REMOVED] definition against documentation
...
1. Replicate sample partitioning (Train[PATH_REMOVED])
...
6. **SHAP local explanations**: waterfall plots for edge-case predictions (top[PATH_REMOVED] deciles, misclassified records)
...
| --- | ------------- | --------------- | -------- | ----------- | -------- |
| 1   | [Description] | High[PATH_REMOVED] | [Domain] | [Action]    | [Date]   |

## Detailed Analysis
### 1. Documentation & Governance - [Pass[PATH_REMOVED]]
### 2. Data Reconstruction - [Pass[PATH_REMOVED]]
### 3. Target / Label Analysis - [Pass[PATH_REMOVED]]
### 4. Segmentation - [Pass[PATH_REMOVED]]
### 5. Feature Analysis - [Pass[PATH_REMOVED]]
### 6. Model Replication - [Pass[PATH_REMOVED]]
### 7. Calibration - [Pass[PATH_REMOVED]]
### 8. Performance & Monitoring - [Pass[PATH_REMOVED]]
### 9. Interpretability & Fairness - [Pass[PATH_REMOVED]]
### 10. Business Impact - [Pass[PATH_REMOVED]]
...
- **Remediation tracking**: 90%+ of High[PATH_REMOVED] findings remediated within deadline
...
- Scheduled PSI[PATH_REMOVED] computation for input and output stability
