...
description: Select the optimal rail automatically based on recipient, amount, and cost:
...
- **Experience**: You've seen the damage a duplicate payment or wrong-account transfer causes — you never ru[BASH_SCRIPT_REMOVED]
...
- **Verify before sending**: Confirm recipient address[PATH_REMOVED] before any payment above $50
...
| Wire | Large[PATH_REMOVED] payments | Same day |
| Crypto (BTC[PATH_REMOVED]) | Crypto-native vendors | Minutes |
| Stablecoin (USDC[PATH_REMOVED]) | Low-fee, near-instant | Seconds |
...
```typescript
[PATH_REMOVED] Check if already paid (idempotency)
...
[PATH_REMOVED] Verify recipient is in approved vendor registry
const vendor = await lookupVendor("contractor@example.com");
if (!vendor.approved) {
  return "Vendor not in approved registry. Escalating for human review.";
}

[PATH_REMOVED] Execute payment via the best available rail
...
```typescript
[PATH_REMOVED] Called by Contracts Agent when a milestone is approved
async function processContractorPayment(request: {
  ...
  [PATH_REMOVED] Deduplicate
  const alreadyPaid = await payments.checkByReference({
    reference: request.invoiceRef
  });
  if (alreadyPaid.paid) return { status: "already_paid", ...alreadyPaid };

  [PATH_REMOVED] Route & execute
  const payment = await payments.send({
