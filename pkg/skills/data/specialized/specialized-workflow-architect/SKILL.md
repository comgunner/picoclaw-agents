...
description: The registry is organized into four cross-referenced views:
...
- **Read every worker[PATH_REMOVED] file.** Every background job type is a workflow.
...
|---|---|---|---|---|---|
| User signup | WORKFLOW-user-signup.md | Approved | POST [PATH_REMOVED] | Auth service | 2026-03-14 |
...
|---|---|---|
| Auth API | src[PATH_REMOVED] | User signup, Password reset, Account deletion |
| Order worker | src[PATH_REMOVED] | Order checkout, Payment processing, Order cancellation |
| Email service | src[PATH_REMOVED] | User signup, Password reset, Order confirmation |
| Database migrations | db[PATH_REMOVED] | All workflows (schema foundation) |
```
...
|---|---|---|
| Signs up for the first time | User signup -> Email verification | [PATH_REMOVED] |
| Completes a purchase | Order checkout -> Payment processing -> Confirmation | [PATH_REMOVED] |
| Deletes their account | Account deletion -> Data cleanup | [PATH_REMOVED] |

### Operator Journeys
| What the operator does | Underlying workflow(s) | Entry point |
|---|---|---|
| Creates a new user manually | Admin user creation | Admin panel [PATH_REMOVED] |
| Investigates a failed order | Order audit trail | Admin panel [PATH_REMOVED]:id |
| Suspends an account | Account suspension | Admin panel [PATH_REMOVED]:id |
...
7. **Concurrent conflicts** (same resource created[PATH_REMOVED] twice simultaneously)
...
- Timeout value
- Recovery action on timeout[PATH_REMOVED]
...
**Implements**: [Issue[PATH_REMOVED] reference]
...
|---|---|---|---|---|
| RC-1 | [Gap or discrepancy found] | Critical[PATH_REMOVED] | [Section] | [Fixed in spec v0.2 / Opened issue #N] |
...
- [ ] CI[PATH_REMOVED] pipeline definitions
...
|---|---|---|---|---|
| 1 | [workflow name] | Yes[PATH_REMOVED] | Critical[PATH_REMOVED] | [notes] |
```
...
```[BASH_SCRIPT_REMOVED]
grep -rn "router\.\(post\|put\|delete\|get\|patch\)" src[PATH_REMOVED] --include="*.ts" --include="*.js"
...
find . -path "*[PATH_REMOVED]*" -type f | head -30

# Find all infrastructure resources
find . -name "*.tf" -o -name "docker-compose*.yml" -o -name "*.yaml" | xargs grep -l "resource\|service:" 2>[PATH_REMOVED]
...
- The **actual implementation** in the relevant workers[PATH_REMOVED] — not just the spec
- Recent git history on the file: `git log --oneline -10 -- path[PATH_REMOVED]`
