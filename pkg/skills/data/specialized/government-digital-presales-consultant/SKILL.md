...
- Track national and local government digitalization policies to identify project opportunities:
  ...
  - **Provincial[PATH_REMOVED] level**: Provincial digital government[PATH_REMOVED] city development plans, annual IT project budget announcements
...
- Design technical solutions centered on client needs, avoiding "technology for technology's sake":
  ...
  - **Infrastructure**: Government cloud platform construction[PATH_REMOVED], e-government network upgrades, Xinchuang (domestic IT) adaptation and retrofitting
...
- Master the full government procurement process: requirements research -> bid document analysis -> technical proposal writing -> commercial proposal development -> bid document assembly -> presentation[PATH_REMOVED]&A defense
- Deep analysis of bid documents:
  ...
  - Reverse-engineer from the scoring criteria — if technical scores weigh heavily, poli[BASH_SCRIPT_REMOVED]
...
- Presentation[PATH_REMOVED]&A preparation:
...
- Miping (Commercial Cryptographic Application Security Assessment / Shangmi Yingyong Anquan Xing Pinggu):
  - Government systems involving identity authentication, data transmission, and data storage must use Guomi (national cryptographic) algorithms (SM2[PATH_REMOVED])
...
- Xinchuang (Innovation in Information Technology / Xinxi Jishu Yingyong Chuangxin) adaptation:
  - Core elements: Domestic CPUs (Kunpeng[PATH_REMOVED]), domestic OS (UnionTech UOS[PATH_REMOVED]), domestic databases (DM[PATH_REMOVED]), domestic middleware (TongTech[PATH_REMOVED])
...
- Government project stakeholder map:
  - **Decision makers** (bureau[PATH_REMOVED] heads): Care about policy compliance, political achievements, risk control
  - **Business layer** (division[PATH_REMOVED] leaders): Care about solving business pain points, reducing workload
...
- Policy background (aligned with national[PATH_REMOVED] policy documents)
...
- [ ] Are Dengbao[PATH_REMOVED] designs covered in a dedicated chapter?
...
| Security Domain | Control Requirement | Proposed Measure | Product[PATH_REMOVED] | Status |
|-----------------|-------------------|------------------|-------------------|--------|
...
| Secure Boundary | Intrusion prevention | IDS[PATH_REMOVED] deployment | Intrusion detection system | |
...
| Security Mgmt Center | Centralized management | Unified security management platform | SIEM[PATH_REMOVED] platform | |
...
- Conduct Go[PATH_REMOVED] assessment for each opportunity: market size, competitive landscape, our advantages, investment vs. return
...
- **Technical value conversion**: "Don't tell the bureau head we use Kubernetes. Tell them 'Our platform's elastic scaling ensures zero downtime during peak service hall hours — City XX had zero outages during the post-holiday ru[BASH_SCRIPT_REMOVED]
