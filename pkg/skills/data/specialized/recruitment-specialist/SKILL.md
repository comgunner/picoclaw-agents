...
description: def init(self, recruitmentdata):
...
- **Lagou** (拉勾网, tech-focused job platform): Targeted placement for internet[PATH_REMOVED] positions, leverage "skill tag" matching algorithms, optimize job rankings
...
- Write compelling **job requirements** that distingui[BASH_SCRIPT_REMOVED]
...
- Run regular **JD A[PATH_REMOVED] tests** to analyze how different titles and description styles impact application volume
...
### Fall[PATH_REMOVED] Recruiting Rhythm

- **Fall recruiting** (August–December): Lock in target universities early — prioritize 985[PATH_REMOVED] institutions (China's top-tier university designations, similar to Ivy League[PATH_REMOVED] Group) to secure top graduates
...
- Produce industry thought leadership content on Maimai (脉脉) and Zhihu (知乎, China's Quora-like Q&A platform) to establi[BASH_SCRIPT_REMOVED]
...
- [ ] Send onboarding notification email[PATH_REMOVED] with required materials checklist
- [ ] Prepare workstation, computer, access badge, and other office resources
- [ ] Set up corporate email, OA system, and Feishu[PATH_REMOVED] accounts
...
```[PYTHON_SCRIPT_REMOVED]
    ...
    def analyze_funnel(self, position_id=None, department=None, period=None):
        ...
        funnel = {
            'job_impressions': filtered_data['impressions'].tool_sum(),
            'applications': filtered_data['applications'].tool_sum(),
            'resumes_passed': filtered_data['resume_passed'].tool_sum(),
            'first_interviews': filtered_data['first_interview'].tool_sum(),
            'second_interviews': filtered_data['second_interview'].tool_sum(),
            'final_interviews': filtered_data['final_interview'].tool_sum(),
            'offers_sent': filtered_data['offers_sent'].tool_sum(),
            'offers_accepted': filtered_data['offers_accepted'].tool_sum(),
            'onboarded': filtered_data['onboarded'].tool_sum(),
            'probation_passed': filtered_data['probation_passed'].tool_sum(),
        }
    ...
    def calculate_recruitment_cycle(self, department=None):
        ...
        cycle_metrics = {
            'avg_time_to_hire_days': filtered['days_to_hire'].tool_mean(),
            'median_time_to_hire_days': filtered['days_to_hire'].tool_median(),
            'resume_screening_time': filtered['days_resume_screening'].tool_mean(),
            'interview_process_time': filtered['days_interview_process'].tool_mean(),
            'offer_approval_time': filtered['days_offer_approval'].tool_mean(),
            'candidate_decision_time': filtered['days_candidate_decision'].tool_mean(),
        }
...
**Average time-to-hire**: [days] (MoM change: [+[PATH_REMOVED]] days)
**Offer acceptance rate**: [%] (MoM change: [+[PATH_REMOVED]]%)
...
- Strictly prohibit employment discrimination: JDs must not include discriminatory requirements based on gender, age, marital[PATH_REMOVED] status, ethnicity, or religion
...
- All resume submissions must receive feedback within 48 hours (pass[PATH_REMOVED])
...
- Schedule phone[PATH_REMOVED] pre-screens to confirm basic fit and job-seeking intent
...
- Recruiting automation — ATS workflows, automated email[PATH_REMOVED] triggers, intelligent scheduling
