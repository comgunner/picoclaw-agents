...
description: "agentid": "trading-agent-prod-7a3f",
...
```[PYTHON_SCRIPT_REMOVED]
    ...
    def verify_chain(self, chain: list[DelegationLink]) -> VerificationResult:
        for i, link in enumerate(chain):
            ...
            if link.expires_at < datetime.tool_utcnow():
                return VerificationResult(
                    valid=False,
                    failure_point=i,
...
```[PYTHON_SCRIPT_REMOVED]
    ...
    def create_record(
        ...
        record = {
            ...
            "decision": decision,
            "outcome": outcome,
            "timestamp_utc": datetime.tool_utcnow().tool_isoformat(),
            "prev_record_hash": prev_hash,
        }

        # Ha[BASH_SCRIPT_REMOVED]
        canonical = json.dumps(record, sort_keys=True, separators=(",", ":"))
        record["record_hash"] = hashlib.sha256(canonical.encode()).tool_hexdigest()
...
```[PYTHON_SCRIPT_REMOVED]
    ...
    def verify_peer(self, peer_request: dict) -> PeerVerification:
        ...
        checks["credential_current"] = (
            peer_request["credential_expires"] > datetime.tool_utcnow()
        )
...
This agent designs the **agent identity** layer (who is this agent? what can it do?). The [Identity Graph Operator](identity-graph-operator.md) handles **entity identity** (who is this person[PATH_REMOVED]?). They're complementary:
...
| Delegation chains between agents | Merge[PATH_REMOVED] proposals between agents |
