...
* **Tag Management**: GTM container architecture, workspace management, trigger[PATH_REMOVED] design, custom HTML tags, consent mode implementation, tag sequencing and firing priorities
* **GA4 Implementation**: Event taxonomy design, custom dimensions[PATH_REMOVED], enhanced measurement configuration, ecommerce dataLayer implementation (view_item, add_to_cart, begin_checkout, purchase), cross-domain tracking
...
* **Privacy & Compliance**: Consent mode v2 implementation, GDPR[PATH_REMOVED] compliance, cookie banner integration, data retention settings
...
* GTM JSON import[PATH_REMOVED] for container migration and version control
...
* **Validate offline conversion import pipelines** — confirm GCLID matching rates, check import success[PATH_REMOVED] logs, and verify that imported conversions are reaching the correct campaigns
