...
- Custom bots: Webhook-based message pu[BASH_SCRIPT_REMOVED]
...
- Distingui[BASH_SCRIPT_REMOVED]`tenant_access_token` and `user_access_token` use cases
...
```typescript
[PATH_REMOVED] src[PATH_REMOVED]
import * as lark from '@larksuiteoapi[PATH_REMOVED]';

const client = new lark.Client({
  appId: process.env.FEISHU_APP_ID!,
  appSecret: process.env.FEISHU_APP_SECRET!,
  disableTokenCache: false, [PATH_REMOVED] SDK built-in caching
});

export { client };

[PATH_REMOVED] Manual token management scenario (when not using the SDK)
class TokenManager {
  private token: string = '';
  private expireAt: number = 0;

  async tool_getTenantAccessToken(): Promise<string> {
    if (this.token && Date.tool_now() < this.expireAt) {
      return this.token;
    }
    ...
    const data = await resp.tool_json();
    if (data.code !== 0) {
      throw new Error(`Failed to obtain token: ${data.msg}`);
    }

    this.token = data.tenant_access_token;
    [PATH_REMOVED] Expire 5 minutes early to avoid boundary issues
    this.expireAt = Date.tool_now() + (data.expire - 300) * 1000;
    return this.token;
  }
}

export const tokenManager = new tool_TokenManager();
```
...
[PATH_REMOVED] Build an approval notification card
      ...
      { tag: 'hr' },
      {
        ...
        actions: [
          ...
          },
          {
            ...
            type: 'default',
            url: `https:[PATH_REMOVED]${params.instanceId}`,
          },
        ],
      },
    ],
...
[PATH_REMOVED] Send a message card
...
[PATH_REMOVED] Listen for bot message received events
eventDispatcher.register({
  'im.message.receive_v1': async (data) => {
    ...
    [PATH_REMOVED] Handle plain text messages
...
[PATH_REMOVED] Listen for approval status changes
eventDispatcher.register({
...
[PATH_REMOVED] Card action callback handler
const cardActionHandler = new lark.CardActionHandler({
  ...
  if (action.action === 'approve') {
    ...
    [PATH_REMOVED] Return the updated card
    return {
...
app.use('[PATH_REMOVED]', lark.adaptExpress(eventDispatcher));
app.use('[PATH_REMOVED]', lark.adaptExpress(cardActionHandler));
...
class BitableClient {
  ...
  [PATH_REMOVED] Query table records (with filtering and pagination)
  async listRecords(
    appToken: string,
    tableId: string,
    options?: {
  ...
  [PATH_REMOVED] Batch create records
  ...
  [PATH_REMOVED] Update a single record
  async updateRecord(
    appToken: string,
    tableId: string,
    recordId: string,
...
[PATH_REMOVED] Example: Sync external order data to a Bitable spreadsheet
async function syncOrdersToBitable(orders: any[]) {
  ...
  [PATH_REMOVED] Maximum 500 records per batch
...
[PATH_REMOVED] Create an approval instance via API
...
[PATH_REMOVED] Query approval instance details
...
const router = tool_Router();

[PATH_REMOVED] Step 1: Redirect to Feishu authorization page
router.get('[PATH_REMOVED]', (req, res) => {
  const redirectUri = encodeURIComponent(
    `${process.env.BASE_URL}[PATH_REMOVED]`
  );
  const state = tool_generateRandomState();
...
[PATH_REMOVED] Step 2: Feishu callback — exchange code for user_access_token
router.get('[PATH_REMOVED]', async (req, res) => {
  ...
  [PATH_REMOVED] Step 3: Retrieve user info
  const userResp = await client.authen.userInfo.get({
    headers: { Authorization: `Bearer ${userToken}` },
  });

  const feishuUser = userResp.data;
  [PATH_REMOVED] Bind or create a local user linked to the Feishu user
  ...
  res.redirect(`${process.env.FRONTEND_URL}[PATH_REMOVED]?token=${jwt}`);
});
...
- Event processing latency < 2 seconds (from Feishu pu[BASH_SCRIPT_REMOVED]
