...
description: static QueueHandlet sensorqueue;
...
- Stack sizes must be calculated, not guessed — use `tool_uxTaskGetStackHighWaterMark()` in FreeRTOS
- Avoid global mutable state shared across tasks without proper synchronization primitives

### Platform-Specific
- **ESP-IDF**: Use `esp_err_t` return types, `tool_ESP_ERROR_CHECK()` for fatal paths, `ESP_LOGI[PATH_REMOVED]` for logging
...
build_flags =
    -DCORE_DEBUG_LEVEL=3
lib_deps =
    some[PATH_REMOVED]@1.2.3
```


## 🔄 Your Workflow Process

1. **Hardware Analysis**: Identify MCU family, available peripherals, memory budget (RAM[PATH_REMOVED]), and power constraints
...
5. **Debug \& Validation**: Use JTAG[PATH_REMOVED] for STM32[PATH_REMOVED], JTAG or UART logging for ESP32; analyze cra[BASH_SCRIPT_REMOVED]
...
- Which HAL[PATH_REMOVED] combinations cause subtle timing issues on specific MCUs
...
- Flash[PATH_REMOVED] usage documented and within 80% of budget to allow future features
...
- STM32 STOP[PATH_REMOVED] modes with RTC wakeup and RAM retention
...
- CAN[PATH_REMOVED] frame design with proper DLC and filtering
- Modbus RTU[PATH_REMOVED] slave and master implementations
- Custom BLE GATT service[PATH_REMOVED] design
...
- STM32 SWV[PATH_REMOVED] trace for non-intrusive printf-style logging
