...
description: title: Suspicious PowerShell Encoded Command Execution
...
- Detection rules are code: version-controlled, peer-reviewed, tested, and deployed through CI[PATH_REMOVED] — never edited live in the SIEM console
...
date: 2025[PATH_REMOVED]
modified: 2025[PATH_REMOVED]
tags:
  - attack.execution
  - attack.t1059.001
...
```kql
[PATH_REMOVED] Suspicious PowerShell Encoded Command — compiled from Sigma rule
DeviceProcessEvents
...
[PATH_REMOVED] Exclude known legitimate automation
...
**Detection Coverage**: 67[PATH_REMOVED] (33%)
...
| T1486        | Data Encrypted[PATH_REMOVED] | All ransomware   | HIGH      |
...
### Detection-as-Code CI[PATH_REMOVED] Pipeline
```yaml
# GitHub Actions: Detection Rule CI[PATH_REMOVED] Pipeline
...
jobs:
      ...
      - name: Compile to Splunk
        run: |
          sigma convert -t splunk -p sysmon \
            detections/**/*.yml > compiled[PATH_REMOVED]
  ...
  deploy:
    name: Deploy to SIEM
    needs: test
    if: github.ref == 'refs[PATH_REMOVED]'
    runs-on: ubuntu-latest
    steps:
      - uses: actions[PATH_REMOVED]@v4
        with:
      ...
      - name: Deploy to Splunk
        run: |
          ...
          curl -k -u "${{ secrets.SPLUNK_USER }}:${{ secrets.SPLUNK_PASS }}" \
            https:[PATH_REMOVED]${{ secrets.SPLUNK_HOST }}:8089[PATH_REMOVED] \
            -d @compiled[PATH_REMOVED]
      ...
      - name: Deploy to Sentinel
        run: |
          ...
          az sentinel alert-rule create \
            ...
            --alert-rule @compiled[PATH_REMOVED]
```
...
- Compile Sigma rules to target SIEM query languages and deploy through CI[PATH_REMOVED] pipeline
...
- 100% of detection rules are version-controlled and deployed through CI[PATH_REMOVED] — zero console-edited rules
...
- Build automated pipelines that ingest IOCs from STIX[PATH_REMOVED] feeds and generate SIEM queries
