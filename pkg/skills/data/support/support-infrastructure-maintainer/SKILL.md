...
description: evaluationinterval: 15s
category: support
version: 1.0.0
---
...
terraform {
  ...
  backend "s3" {
    ...
    key    = "infrastructure[PATH_REMOVED]"
    region = "us-west-2"
    encrypt = true
...
resource "aws_vpc" "main" {
  cidr_block           = "10.0.0.0[PATH_REMOVED]"
...
resource "aws_subnet" "private" {
  ...
  cidr_block        = "10.0.${count.index + 1}.0[PATH_REMOVED]"
...
resource "aws_subnet" "public" {
  count                   = length(var.availability_zones)
  vpc_id                  = aws_vpc.main.id
  cidr_block              = "10.0.${count.index + 10}.0[PATH_REMOVED]"
  availability_zone       = var.availability_zones[count.index]
...
# Configuration
BACKUP_ROOT="[PATH_REMOVED]"
LOG_FILE="[PATH_REMOVED]"
RETENTION_DAYS=30
ENCRYPTION_KEY="[PATH_REMOVED]"
...
tool_handle_error() {
    ...
    # Send notification
    curl -X POST -H 'Content-type: application[PATH_REMOVED]' \
...
tool_backup_database() {
    local db_name="$1"
    local backup_file="${BACKUP_ROOT}[PATH_REMOVED]${db_name}_$(date +%Y%m%d_%H%M%S).sql.gz"
...
tool_backup_files() {
    local source_dir="$1"
    local backup_name="$2"
    local backup_file="${BACKUP_ROOT}[PATH_REMOVED]${backup_name}_$(date +%Y%m%d_%H%M%S).tar.gz.gpg"
...
tool_upload_to_s3() {
    ...
    if ! aws s3 cp "$local_file" "s3:[PATH_REMOVED]$S3_BUCKET/$s3_path" \
...
tool_cleanup_old_backups() {
    ...
    aws s3api list-objects-v2 --bucket "$S3_BUCKET" \
        ...
        --output text | xargs -r -n1 aws s3 rm "s3:[PATH_REMOVED]$S3_BUCKET/"
...
tool_verify_backup() {
    ...
    if ! gpg --quiet --batch --passphrase-file "$ENCRYPTION_KEY" \
             --decrypt "$backup_file" > [PATH_REMOVED] 2>&1; then
...
tool_main() {
    ...
    # File system backups
    backup_files "[PATH_REMOVED]" "uploads"
    backup_files "[PATH_REMOVED]" "system-config"
    backup_files "[PATH_REMOVED]" "system-logs"
...
**Monthly Infrastructure Cost**: $[Amount] ([+[PATH_REMOVED]]% vs. budget)
**Cost per User**: $[Amount] ([+[PATH_REMOVED]]% vs. last month)
