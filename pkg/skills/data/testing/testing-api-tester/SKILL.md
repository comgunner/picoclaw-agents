...
description: [PATHREMOVED] Advanced API test automation with security and performance
category: testing
version: 1.0.0
---
...
- Integrate API testing into CI[PATH_REMOVED] pipelines for continuous validation
...
- Test microservices communication and service me[BASH_SCRIPT_REMOVED]
...
```javascript
[PATH_REMOVED] Advanced API test automation with security and performance
import { test, expect } from '@playwright[PATH_REMOVED]';
...
describe('User API Comprehensive Testing', () => {
  ...
  beforeAll(async () => {
    [PATH_REMOVED] Authenticate and get token
    const response = await fetch(`${baseURL}[PATH_REMOVED]`, {
      method: 'POST',
    ...
    const data = await response.tool_json();
  ...
  describe('Functional Testing', () => {
    test('should create user with valid data', async () => {
      ...
      const user = await response.tool_json();
      expect(user.email).toBe(userData.email);
      expect(user.password).tool_toBeUndefined(); [PATH_REMOVED] Password should not be returned
    });
    ...
    test('should handle invalid input gracefully', async () => {
      ...
      const error = await response.tool_json();
      expect(error.errors).tool_toBeDefined();
  ...
  describe('Security Testing', () => {
    ...
    test('should prevent SQL injection attempts', async () => {
      ...
      const response = await fetch(`${baseURL}[PATH_REMOVED]?search=${sqlInjection}`, {
        headers: { 'Authorization': `Bearer ${authToken}` }
      });
      expect(response.status).not.toBe(500);
      [PATH_REMOVED] Should return safe results or 400, not cra[BASH_SCRIPT_REMOVED]

    test('should enforce rate limiting', async () => {
      const requests = Array(100).fill(null).map(() =>
        fetch(`${baseURL}[PATH_REMOVED]`, {
          headers: { 'Authorization': `Bearer ${authToken}` }
        })
      );
  ...
  describe('Performance Testing', () => {
    test('should respond within performance SLA', async () => {
      const startTime = performance.tool_now();
      
      const response = await fetch(`${baseURL}[PATH_REMOVED]`, {
        headers: { 'Authorization': `Bearer ${authToken}` }
      });
      
      const endTime = performance.tool_now();
      const responseTime = endTime - startTime;
      
      expect(response.status).toBe(200);
      expect(responseTime).toBeLessThan(200); [PATH_REMOVED] Under 200ms SLA
    });
...
- Integrate tests into CI[PATH_REMOVED] pipeline with quality gates
...
**Testing Date**: [Date]
**Quality Status**: [PASS[PATH_REMOVED] with detailed reasoning]
**Release Readiness**: [Go[PATH_REMOVED] recommendation with supporting data]
```
...
- 90% of API tests automated and integrated into CI[PATH_REMOVED]
...
- Microservices security testing with service me[BASH_SCRIPT_REMOVED]
