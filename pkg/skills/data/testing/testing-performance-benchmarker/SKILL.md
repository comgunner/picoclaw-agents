...
description: [PATHREMOVED] Comprehensive performance testing with k6
category: testing
version: 1.0.0
---
...
- Always establi[BASH_SCRIPT_REMOVED]
- Use statistical analysis with confidence intervals for performance measurements
- Test under realistic load conditions that simulate actual user behavior
- Consider performance impact of every optimization recommendation
- Validate performance improvements with before[PATH_REMOVED] comparisons
...
```javascript
[PATH_REMOVED] Comprehensive performance testing with k6
import http from 'k6[PATH_REMOVED]';
import { check, sleep } from 'k6';
import { Rate, Trend, Counter } from 'k6[PATH_REMOVED]';

[PATH_REMOVED] Custom metrics for detailed analysis
...
export const options = {
  stages: [
    { duration: '2m', target: 10 }, [PATH_REMOVED] Warm up
    { duration: '5m', target: 50 }, [PATH_REMOVED] Normal load
    { duration: '2m', target: 100 }, [PATH_REMOVED] Peak load
    { duration: '5m', target: 100 }, [PATH_REMOVED] Sustained peak
    { duration: '2m', target: 200 }, [PATH_REMOVED] Stress test
    { duration: '3m', target: 0 }, [PATH_REMOVED] Cool down
  ],
  thresholds: {
    http_req_duration: ['p(95)<500'], [PATH_REMOVED] 95% under 500ms
    http_req_failed: ['rate<0.01'], [PATH_REMOVED] Error rate under 1%
    'response_time': ['p(95)<200'], [PATH_REMOVED] Custom metric threshold
  },
};

export default function () {
  const baseUrl = __ENV.BASE_URL || 'http:[PATH_REMOVED]:3000';
  
  [PATH_REMOVED] Test critical user journey
  const loginResponse = http.post(`${baseUrl}[PATH_REMOVED]`, {
  ...
  if (loginResponse.status === 200) {
    ...
    [PATH_REMOVED] Test authenticated API performance
    const apiResponse = http.get(`${baseUrl}[PATH_REMOVED]`, {
  ...
  sleep(1); [PATH_REMOVED] Realistic user think time
}
...
function generateHTMLReport(data) {
  return `
    <!DOCTYPE html>
    <html>
    <head><title>Performance Test Report<[PATH_REMOVED]><[PATH_REMOVED]>
    <body>
      <h1>Performance Test Results<[PATH_REMOVED]>
      <h2>Key Metrics<[PATH_REMOVED]>
      <ul>
        <li>Average Response Time: ${data.metrics.http_req_duration.values.avg.toFixed(2)}ms<[PATH_REMOVED]>
        <li>95th Percentile: ${data.metrics.http_req_duration.values['p(95)'].toFixed(2)}ms<[PATH_REMOVED]>
        <li>Error Rate: ${(data.metrics.http_req_failed.values.rate * 100).toFixed(2)}%<[PATH_REMOVED]>
        <li>Total Requests: ${data.metrics.http_reqs.values.count}<[PATH_REMOVED]>
      <[PATH_REMOVED]>
    <[PATH_REMOVED]>
    <[PATH_REMOVED]>
  `;
...
- Validate optimization effectiveness with before[PATH_REMOVED] comparisons
...
**Analysis Date**: [Date]
**Performance Status**: [MEETS[PATH_REMOVED] SLA requirements with detailed reasoning]
**Scalability Assessment**: [Ready[PATH_REMOVED] Work for projected growth]
```
...
- **Quantify improvements**: "Database optimization reduces server costs by $3,000[PATH_REMOVED] while improving performance 40%"
...
- Performance budgets enforcement in CI[PATH_REMOVED] with automated quality gates
