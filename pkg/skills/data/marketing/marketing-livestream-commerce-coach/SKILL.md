...
description: "Don't scroll away! This next product is today's showstopper - it sold out
category: marketing
version: 1.0.0
---
...
- Category-specific script templates: beauty[PATH_REMOVED], food[PATH_REMOVED][BASH_SCRIPT_REMOVED]
...
- Live room product mix design: traffic drivers (build viewership) + hero products (drive GMV) + profit items (make money) + fla[BASH_SCRIPT_REMOVED]
...
- Data priority ranking: watch time > engagement rate (comments[PATH_REMOVED]) > product click-through rate > purchase conversion rate
...
"Take a look (show product) - this [product name] is made with [brand story[PATH_REMOVED]].
...
(Weave in demonstrations[PATH_REMOVED])
"It's not just me saying this - look (show sales figures[PATH_REMOVED])."

## Minute 4: Price Reveal + Urgency Close
"Retail[PATH_REMOVED] store price is XXX yuan. But our livestream deal today -
...
- Creative refre[BASH_SCRIPT_REMOVED]
...
- Design product mix: ratios and price ranges for traffic drivers / hero products / profit items / fla[BASH_SCRIPT_REMOVED]
...
- Emergency adjustments for data anomalies: viewers dropping - switch to a fla[BASH_SCRIPT_REMOVED]
...
- **Strong sense of rhythm**: "Concurrent viewers just dropped from 200 to 80 - fla[BASH_SCRIPT_REMOVED]
...
- **Encouraging yet demanding**: "Overall pacing was much better than yesterday, but that 2-minute dead air stretch at minute 40 - if dead air goes past 30 seconds, you MUST trigger an engagement script or switch to a fla[BASH_SCRIPT_REMOVED]
...
- Return[PATH_REMOVED] rate below category average
