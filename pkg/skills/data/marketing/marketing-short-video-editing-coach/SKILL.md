...
description: Specialized AI skill for professional tasks
category: marketing
version: 1.0.0
---
...
- **Adobe Premiere Pro**
  ...
  - Key strengths: Industry standard, seamless integration with AE[PATH_REMOVED], richest plug-in ecosystem, best multi-format compatibility
...
- **Camera movements**
  ...
  - Pan: Horizontal[PATH_REMOVED] rotation; shows full spatial context; suits environment introductions and scene transitions
...
- **Transition design**
  ...
  - Dissolve (cross-fade): Two shots fade in[PATH_REMOVED] overlapping; conveys time passage or emotional transition
  ...
  - Zoom transition: Rapid zoom in[PATH_REMOVED] creates a "warp" effect
  - Fla[BASH_SCRIPT_REMOVED]
...
- **Primary correction - restoring reality**
  - White balance: Color temperature (warm[PATH_REMOVED]) and tint (green[PATH_REMOVED]); ensure white is actually white
...
- **Secondary correction - targeted refinement**
  - HSL adjustment: Independently adjust hue[PATH_REMOVED] of specific colors (e.g., making only the sky bluer)
...
- **Proper LUT usage**
  ...
  - Usage principle: A LUT is a starting point, not the fini[BASH_SCRIPT_REMOVED]
...
- **Stylistic grading directions**
  ...
  - Cyberpunk: High-saturation neon (magenta[PATH_REMOVED]) + high contrast + crushed blacks
  - Vintage film: Yellow-green tint + reddi[BASH_SCRIPT_REMOVED]
...
- **BGM beat-syncing**
  - Rhythm markers: Listen through the BGM to find downbeats[PATH_REMOVED]; mark them on the timeline
  - Visual beat-sync: Cut shots on downbeats[PATH_REMOVED] for audiovisual impact
...
- **Keyframe animation**
  ...
  - Easing curves (the critical detail): Linear motion looks "mechanical"; ease-in[PATH_REMOVED] makes it natural - Bezier curves are the soul
...
- **Speed curves (speed ramping)**
  ...
  - Frame rate requirement: Shoot at 60fps or 120fps for smooth slow-motion; 24[PATH_REMOVED] footage will stutter when slowed
...
- **Decorative text (fancy subs)**
  ...
  - Layering: Bottom layer stroke[PATH_REMOVED] + middle layer color fill + top layer highlight[PATH_REMOVED]; aim for at least two layers
...
- **Variety-show subtitle style**
  ...
  - Color rules: Different speakers get different colors; keywords pop in attention-grabbing colors (red[PATH_REMOVED])
...
- **Multilingual subtitles**
  ...
  - ASS format: Supports rich styling (font[PATH_REMOVED]); commonly used for Bilibili uploads
...
- **Vertical 9:16 (Douyin / Kuaishou / Channels / Xiaohongshu)**
  ...
  - Frame rate: 30fps (standard) or 60fps (sports[PATH_REMOVED] content)
  - Bitrate recommendation: 1080p at 8-15Mbps; 4K at 20-35Mbps
  - Duration strategy: Douyin 7-15s (entertainment) / 1-3min (educational[PATH_REMOVED]); Kuaishou (short-video platform) 15-60s; Xiaohongshu (lifestyle platform) 1-5min
...
- **Horizontal 16:9 (Bilibili / YouTube / Xigua Video)**
  ...
  - Frame rate: 24fps (cinematic), 30fps (standard), 60fps (gaming[PATH_REMOVED])
...
- **Thumbnail design**
  ...
  - Horizontal thumbnail composition: Text-left[PATH_REMOVED] or text-top[PATH_REMOVED]; key info centered or slightly above center
  ...
  - A[PATH_REMOVED] testing: Prepare 2-3 different thumbnails per video; track CTR data post-publi[BASH_SCRIPT_REMOVED]
...
- **Asset management**
  - Folder structure: Organize by project / date / asset type (video[PATH_REMOVED] files) in hierarchical directories
  - File naming convention: date_project_shot-number_description, e.g., "20260312_product-review_S01_unboxing-closeup"
  - Proxy editing: Generate low-resolution proxy files from 4K[PATH_REMOVED] raw footage for editing, then relink to originals for final export - this is a lifesaving technique for high-res workflows
  - Backup strategy: 3-2-1 rule - 3 copies, 2 different storage media, 1 off-site backup
  - Asset tagging and rating: Preview all footage after import, rate shot quality (good[PATH_REMOVED]) to avoid hunting during editing

- **Template-based batch production**
  - Project templates: Preset timeline track layouts, frequently used color presets, subtitle styles, intro[PATH_REMOVED] sequences
...
- **Team collaboration**
  ...
  - Version control: Save as new version for every major revision (v1[PATH_REMOVED]); never overwrite the original file
...
- **Keyboard shortcut efficiency**
  ...
  - Essential shortcuts (PR example): Q[PATH_REMOVED] (ripple edit), J[PATH_REMOVED] (playback control), C (razor), V (selection), I[PATH_REMOVED] (in[PATH_REMOVED] points)
  - Custom shortcuts: Bind most-used operations to left-hand keys (since right hand stays on the mouse)
  - Mouse recommendation: Use a mouse with programmable side buttons; bind undo[PATH_REMOVED] to them
...
- **AI smart cutout**
  ...
  - Edge quality: Hair, semi-transparent objects (glass[PATH_REMOVED]) remain challenging for AI; manual touchup needed when critical
...
- Audiences will tolerate average visuals but cannot stand har[BASH_SCRIPT_REMOVED]
...
- Evaluate asset quality: check resolution[PATH_REMOVED] rate[PATH_REMOVED]; determine if reshoots are needed
- Develop editing plan: establi[BASH_SCRIPT_REMOVED]
...
- Visual technical standards met: no blown highlights[PATH_REMOVED] shadows, no focus misses, no audio-video desync
- Audio quality standards met: clear voice with no background noise, balanced BGM levels, no clipping distortion
- Consistent color grading: videos in the same series[PATH_REMOVED] maintain uniform color style
