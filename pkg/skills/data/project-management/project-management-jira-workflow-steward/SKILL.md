...
description: | Change Type | Branch Pattern | Commit Pattern | When to Use |
...
- Working branches must follow repository intent: `feature[PATH_REMOVED]`, `bugfix[PATH_REMOVED]`, or `hotfix[PATH_REMOVED]`
...
- Release preparation uses `release[PATH_REMOVED]`; release commits should still reference the release ticket or change-control item when one exists
- Commit messages stay on one line and follow `<gitmoji> JIRA-ID: short description`
- Choose Gitmojis from the official catalog first: [gitmoji.dev](https:[PATH_REMOVED]) and the source repository [carloscuesta[PATH_REMOVED]](https:[PATH_REMOVED])
...
|-------------|----------------|----------------|-------------|
| Feature | `feature[PATH_REMOVED]` | `✨ JIRA-214: add SSO login flow` | New product or platform capability |
| Bug Fix | `bugfix[PATH_REMOVED]` | `🐛 JIRA-315: fix token refre[BASH_SCRIPT_REMOVED]` | Non-production-critical defect work |
| Hotfix | `hotfix[PATH_REMOVED]` | `🐛 JIRA-411: patch auth bypass check` | Production-critical fix from `main` |
| Refactor | `feature[PATH_REMOVED]` | `♻️ JIRA-522: refactor audit service boundaries` | Structural cleanup tied to a tracked task |
| Docs | `feature[PATH_REMOVED]` | `📚 JIRA-623: document API error catalog` | Documentation work with a Jira task |
| Tests | `bugfix[PATH_REMOVED]` | `🧪 JIRA-724: add session timeout regression tests` | Test-only change tied to a tracked defect or feature |
| Config | `feature[PATH_REMOVED]` | `🔧 JIRA-811: add branch policy validation` | Configuration or workflow policy changes |
| Dependencies | `bugfix[PATH_REMOVED]` | `📦 JIRA-902: upgrade GitHub Actions versions` | Dependency or platform upgrades |

If a higher-priority tool requires an outer prefix, keep the repository branch intact inside it, for example: `codex[PATH_REMOVED]`.

### Official Gitmoji References
- Primary reference: [gitmoji.dev](https:[PATH_REMOVED]) for the current emoji catalog and intended meanings
- Source of truth: [github.com[PATH_REMOVED]](https:[PATH_REMOVED]) for the upstream project and usage model
...
if [[ ! "$branch" =~ $branch_regex ]]; then
  ...
  echo "Use feature[PATH_REMOVED], bugfix[PATH_REMOVED], hotfix[PATH_REMOVED], or release[PATH_REMOVED]" >&2
  exit 1
fi
...
Implements **JIRA-214** by adding the SSO login flow and tightening token refre[BASH_SCRIPT_REMOVED]

## Jira Link
- Ticket: JIRA-214
- Branch: feature[PATH_REMOVED]

## Change Summary
- Add SSO callback controller and provider wiring
- Add regression coverage for expired refre[BASH_SCRIPT_REMOVED]
...
## Ticket
- Jira: JIRA-315
- Outcome: Fix token refre[BASH_SCRIPT_REMOVED]

## Planned Branch
- bugfix[PATH_REMOVED][BASH_SCRIPT_REMOVED]
1. 🐛 JIRA-315: fix refre[BASH_SCRIPT_REMOVED]
2. 🧪 JIRA-315: add concurrent refre[BASH_SCRIPT_REMOVED]
3. 📚 JIRA-315: document token refre[BASH_SCRIPT_REMOVED]

## Review Notes
- Risk area: authentication and session expiry
- Security check: confirm no sensitive tokens appear in logs
- Rollback: revert commit 1 and disable concurrent refre[BASH_SCRIPT_REMOVED]
```
