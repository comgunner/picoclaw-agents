...
```bash
# 質問する
docker compose run --rm picoclaw-agent -m "2+2は？"
...
# 構成済みテンプレート:
picoclaw onboard --openai      # OpenAI テンプレートを使用 (o3-mini)
picoclaw onboard --openrouter  # OpenRouter テンプレートを使用 (openrouter/auto)
...
Telegram、Discord、DingTalk、LINE、WeCom を通じて picoclaw と話しましょう。
...
> `picoclaw auth login --provider anthropic` を実行して、API トークンを貼り付けます。
...
| コマンド                  | 説明                                       |
| ------------------------- | ------------------------------------------ |
| `picoclaw onboard`        | 構成とワークスペースを初期化する           |
| `picoclaw agent -m "..."` | エージェントとチャットする                 |
| `picoclaw agent`          | 対話型チャットモード                       |
| `picoclaw gateway`        | ゲートウェイを起動する                     |
| `picoclaw status`         | ステータスを表示する                       |
| `picoclaw cron list`      | スケジュールされたすべてのジョブを一覧表示 |
| `picoclaw cron add ...`   | スケジュールされたジョブを追加する         |
...
`mcp_servers` 設定例 (インストール/onboard で生成された `picoclaw` の絶対パスを使用):
...
これは、ボットの別のインスタンスが実行されているときに発生します。一度に実行されている `picoclaw gateway` が1つだけであることを確認してください。
