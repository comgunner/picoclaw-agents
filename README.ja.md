...
2026-03-27 🎉 **WebUI Launcher 完全動作**: `picoclaw-agents-launcher` が end-to-end で動作 — Start Gateway ボタン、PicoChannel 経由の WebSocket チャット、Skills ページのネイティブスキルコンテンツ、すべてのセクション検証済み。`picoclaw-agents-launcher` またはネットワークアクセス用 `picoclaw-agents-launcher -public` で実行。

2026-03-27 🎉 **3 バイナリ リリースパイプライン**: GoReleaser が 3 つのバイナリを生成 — `picoclaw-agents` (CLI)、`picoclaw-agents-launcher` (WebUI)、`picoclaw-agents-launcher-tui` (TUI) — オリジナルプロジェクトのリリース構造に一致。`./scripts/create-release.sh` でトリガー。

2026-03-26 🎉 **MCP Builder ドキュメント**: MCP Builder Agent の完全なドキュメントを英語とスペイン語で提供。API リファレンス、ユースケース、例を含む。[docs/MCP_BUILDER_AGENT.md](docs/MCP_BUILDER_AGENT.md) および [docs/MCP_BUILDER_AGENT.es.md](docs/MCP_BUILDER_AGENT.es.md) を参照。

2026-03-26 🎉 **Sandbox & Codegen コマンド**: 分離ワークスペース用に `sandbox init/status`、Go ボイラープレート生成用に `util codegen` を追加。完全な詳細は [CHANGELOG.md](CHANGELOG.md) を参照。

2026-03-26 🎉 **Auth Token Monitor**: OAuth トークン有効期限追跡用に `auth tokens` および `auth monitor` コマンドを追加。[CHANGELOG.md](CHANGELOG.md) を参照。

2026-03-26 🎉 **Config Validator & Secret Masking**: スキーマ検証および onboard wizard の秘密マスキング用に `config validate` コマンドを追加。[CHANGELOG.md](CHANGELOG.md) を参照。

2026-03-26 🎉 **Doctor Command**: WSL 検出およびセキュリティチェックを含む環境診断用に `doctor` コマンドを追加。[CHANGELOG.md](CHANGELOG.md) を参照。

2026-03-12 🎉 **Antigravity サポート & 安定性**: スキーマサニタイゼーション、TokenBudget デッドロック修正、セッション再水和改善、新しい `picoclaw-agents clean` コマンド、および強化された deny パターンによる完全な Google Antigravity OAuth サポート。完全な詳細は [CHANGELOG.md](CHANGELOG.md) を参照。

2026-03-03 🎉 **ネイティブスキルアーキテクチャ**: バイナリに直接コンパイルされたネイティブスキル (`pkg/skills/queue_batch.go`) を導入し、外部 `.md` ファイル依存関係を排除。セキュリティ、パフォーマンス、型の安全性を強化。[docs/QUEUE_BATCH.en.md](docs/QUEUE_BATCH.en.md) を参照。

2026-03-02 🎉 **Fast-path & グローバルトラッカー**: ゼロレイテンシー対話用のインスタント Slash Commands (`/bundle_approve`, `/status` など) を追加。完全なマルチエージェント状態一貫性のために `ImageGenTracker` を全エージェントで統合。[docs/queue_batch.md](docs/queue_batch.md) を参照。

2026-03-01 🎉 **AI イメージ生成 & コミュニティマネージャー**: **Antigravity OAuth** (`gemini-3.1-flash-image`、API キー不要、300 秒クールダウン)、**Gemini API キー**、または **Ideogram API キー** を使用したネイティブイメージ生成、スクリプトからイメージへのワークフロー、対話型生成後メニュー、およびソーシャルメディア投稿を自動生成するコミュニティマネージャーエージェントを追加。完全な設定と使用例は [docs/IMAGE_GEN_util.md](docs/IMAGE_GEN_util.md) を参照。

2026-03-01 🎉 **外部統合 (Binance, Social Media, Notion)**: 暗号通貨取引 (Binance futures & spot)、ソーシャルメディア投稿 (Facebook & X/Twitter)、および知識管理 (Notion) 用のネイティブツールを追加。`config.json` または環境変数で設定。セットアップガイドは [SOCIAL_MEDIA.md](SOCIAL_MEDIA.md) および [docs/NOTION_util.md](docs/NOTION_util.md) を参照。

2026-03-01 🎉 **Native Skills Sentinel**: プロンプトインジェクションおよびシステムリークに対するリアルタイムのパターンベースの保護を提供する内部セキュリティレイヤー (`skills_sentinel.go`) を追加。
2026-03-01 🎉 **セキュリティと安定性の強化**: ロバストなメッセージバスクローズハンドリング、弾力性のある WeCom App バックグラウンド処理、およびシェルツールの初期化検証の強化。
2026-03-01 🎉 **Fail-Close Security**: ロバストなセキュリティポリシー。コマンド実行ツールは初期化中に deny パターンの厳密な検証を実行するようになりました。

2026-02-27 🎉 **災害復旧 & タスクロック**: エージェントの衝突を防ぐためのアトミックタスクロック、急激なクラッシュからの回復のための「Boot Rehydration」、および長期コーディングタスクでのコンテキスト爆発を根絶するためのコンテキストコンパクター（32K トークンへの安全なリミット引き上げ）を実装。
