...
[English](README.md) | [中文](README.zh.md) | [Français](README.fr.md) | [日本語](README.ja.md) | [Português](README.pt-br.md) | [Tiếng Việt](README.vi.md)
...
- [🖥️ Desplegar en VPS](#️-desplegar-en-vps)
- [🔄 Migración](#-migración-desde-openclaw-o-nanoclaw)
- [🧠 Skills Nativos](#-skills-nativos-opcional)
- [📁 Skills de Workspace](#-skills-de-workspace-personalizados)
- [🔀 Cascada de Modelos Gratuitos](#-cascada-de-modelos-gratuitos-mecanismo-de-supervivencia)
...
Este proyecto se desarrolla y mantiene con la ayuda de herramientas de IA.

**🌟 OpenRouter (Gói Miễn Phí)** — Acceso a más de 200 modelos con cascade fallback. Recomendado.
...
🆓 **Modelos gratis** (prioridad de cascada):
- `openrouter/free` — Selección automática del mejor modelo gratis
- `nvidia/nemotron-3-ultra-550b-a55b:free` — 550B parámetros
- `openai/gpt-oss-20b:free` — GPT Open Source
- `inclusionai/ling-3.0-tiny:free` — Ligero y rápido
...
**Generación de imágenes**: `krea/krea-2-medium-turbo`

👉 [Únete ahora](https://openrouter.ai/)

**🌟 OpenCode Zen (8 Modelos Gratis)** — Sin tarjeta de crédito.
...
- `opencode/ling-3.0-tiny-free` — Ligero y rápido
- `opencode/north-mini-code-free` — Enfocado en código
- `opencode/laguna-s-2.1-free` — Nuevo
- `opencode/longcat-2.0-free` — Nuevo
- `opencode/big-pickle` — Prueba gratuita

👉 [Obtener API Key](https://opencode.ai/auth)
...
🚀 Estás invitado a unirte al GLM Coding Plan. Soporte completo para Claude Code, Cline y más de 20 herramientas de codificación principales — solo $10/mes.
👉 [Únete ahora](https://z.ai/subscribe?ic=RF2YMCHBHL)

**🌟 Qwen Code (Alibaba Cloud)** — Asistente de codificación IA (planes de pago disponibles).
...
> ℹ️ **Nota:** Usa `onboard` para la primera configuración (configura todo automáticamente). Usa `auth` solo si necesitas agregar otra API key a una configuración existente.
>
> **Ejemplo:** Ya ejecutaste `onboard --openrouter` y quieres agregar OpenCode Zen:
> ```bash
> # Primera vez (configura OpenRouter como predeterminado)
> ./picoclaw-agents onboard --openrouter
>
> # Después, agrega OpenCode Zen
> ./picoclaw-agents auth login --provider opencode
>
> # Ahora puedes cambiar entre ellos
> /model opencode/mimo-v2.5-free    # Usar modelo OpenCode
> /model openrouter/free            # Volver a OpenRouter
> ```
>
...
## 🖥️ Gestión de Servicio del SO

Ejecuta PicoClaw-Agents como un servicio del sistema que se inicia automáticamente al hacer login. Compatible con **Linux (systemd)**, **macOS (launchd)** y **Windows (Tareas Programadas)**.

> **Nota:** El subcomando `service` está en el **binario CLI principal**, no en el launcher.

```bash
# ✅ Correcto — binario CLI principal
./build/picoclaw-agents-darwin-arm64 service install

# ❌ Incorrecto — el launcher es solo WebUI
...
| Comando | Descripción |
|---------|-------------|
| `service install` | Instalar como servicio del SO (inicio automático al login) |
| `service install --dry-run` | Previsualizar sin realizar cambios |
| `service install --port 9999` | Instalar en puerto personalizado |
| `service install --public` | Escuchar en todas las interfaces (0.0.0.0) |
| `service uninstall` | Eliminar el servicio |
| `service start` | Iniciar el servicio |
| `service stop` | Detener el servicio |
| `service restart` | Reiniciar el servicio |
| `service status` | Ver estado del servicio |
| `service logs` | Ver logs (últimas 50 líneas) |
| `service logs -n 100` | Ver últimas 100 líneas |
| `service logs -f` | Seguir logs en tiempo real |

### Detalles por Plataforma

| Plataforma | Tipo de Servicio | Ubicación de Configuración |
|----------|-------------|-----------------|
| **Linux** | systemd (usuario) | `~/.config/systemd/user/picoclaw-agents.service` |
| **macOS** | launchd LaunchAgent | `~/Library/LaunchAgents/com.picoclaw.agents.gateway.plist` |
| **Windows** | Tarea Programada (ONLOGON) | `%APPDATA%\PicoClaw\gateway.cmd` |

### Flujo de Uso Típico

```bash
# 1. Instalar el servicio (inicio automático al login)
picoclaw-agents service install

# 2. Abrir la WebUI para configuración visual
picoclaw-agents-launcher --public
# → http://localhost:18800

# 3. El servicio gestiona el gateway automáticamente
# No es necesario lanzarlo manualmente
```

### ⚠️ Prevenir Conexiones Duplicadas

El servicio y un gateway iniciado manualmente **NO deben ejecutarse simultáneamente**. Si ambos están activos, recibirás mensajes duplicados en Telegram/Discord y conflictos de puerto. El instalador verifica si el puerto del gateway ya está en uso y rechaza la instalación si detecta un gateway en ejecución.
...
Plataforma:     macOS (launchd)
Puerto:         18800
Público:        false
Config:         ~/.picoclaw/config.json

Verificaciones previas:
  ✓ Puerto 18800 disponible
  ✓ Servicio no instalado previamente
  ✓ Binario existe

Qué se haría:
  1. Crear directorio: ~/Library/Logs/picoclaw-agents
  2. Escribir archivo plist: ~/Library/LaunchAgents/com.picoclaw.agents.gateway.plist
  3. Ejecutar: launchctl bootstrap gui/$(id -u)/ ...
```

📖 **Documentación completa:** Ver [docs/SERVICE.md](docs/SERVICE.md) para guías de instalación, solución de problemas y detalles específicos de cada plataforma.
...
picoclaw-agents onboard --openrouter  # Usar plantilla de OpenRouter (openrouter/free, por defecto)
...
|-----------|--------|----------|-------|
| Principal | `openrouter/free` | variable | Selecciona automáticamente el mejor modelo gratuito disponible **(por defecto)** |
...
> [!IMPORTANT]
> **Fix de ID de Modelo:** Versiones anteriores usaban `openrouter-free` que no es un ID válido. Ahora `openrouter/free` es el modelo por defecto para nuevas instalaciones. Si tienes una config existente con `openrouter-free`, actualízala a `openrouter/free` o vuelve a ejecutar `picoclaw-agents onboard --free`.
...
### 📁 Skills de Workspace (Personalizados)

Los skills de workspace son skills basados en archivos almacenados en `~/.picoclaw/workspace/skills/`. Son descubiertos automaticamente por el gateway.

**Skills de workspace disponibles:**

| Skill | Descripcion |
|-------|-------------|
| `ui-ux-pro-max` | Inteligencia de diseno UI/UX: 84 estilos, 192 paletas de colores, 74 combinaciones de fuentes, 22 stacks |
...
---
name: mi-skill
description: "Descripcion de mi skill personalizado"
---
# Contenido del skill aqui
EOF

# Reiniciar gateway para descubrir skill
sudo systemctl restart picoclaw.service
```

Ver [`local_work/HOW_TO_ADD_CUSTOM_SKILLS.md`](local_work/HOW_TO_ADD_CUSTOM_SKILLS.md) para instrucciones detalladas.

### 🔀 Cascada de Modelos Gratuitos (Mecanismo de Supervivencia)

PicoClaw usa un **sistema de fallback en cascada** para mantener tu bot en linea 24/7. Cuando el modelo gratuito primario falla, automaticamente intenta el siguiente modelo.

**Modelos gratuitos (sin tarjeta de credito):**

| Prioridad | Modelo | Proveedor |
|-----------|--------|-----------|
| 1 | `openrouter/free` | OpenRouter (selecciona automaticamente el mejor disponible) |
...
Ver [`docs/FREE_MODELS_CASCADE_GUIDE.es.md`](docs/FREE_MODELS_CASCADE_GUIDE.es.md) para configuracion y solucion de problemas.
...
Ejecuta PicoClaw-Agents en un VPS en la nube para disponibilidad 24/7. Especificaciones recomendadas:

| Especificación | Valor |
|----------------|-------|
| **Proveedor** | [Vultr](https://www.vultr.com/?ref=9916942) |
| **Instancia** | vc2-1c-2gb |
| **CPU** | 1 vCPU |
| **RAM** | 2 GB |
| **Almacenamiento** | 55 GB |
| **Precio** | Desde $10/mes |

> 💡 [**Desplegar en Vultr**](https://www.vultr.com/?ref=9916942) — Suficiente para PicoClaw-Agents con modelos gratuitos OpenRouter.
...
| `picoclaw-agents auth login --provider <nombre>` | Iniciar sesión en proveedor |
| `picoclaw-agents auth models`    | Listar modelos disponibles          |
| `picoclaw-agents auth logout --provider <nombre>` | Cerrar sesión |
...
2026-08-09 🎉 **Integración OpenRouter + OpenCode Zen**: Sistema completo de cascade fallback (gratuito→pago) con 15 modelos configurados. OpenRouter recomendado con 5+ modelos gratuitos. OpenCode Zen agregado con 8 modelos gratuitos (MiMo V2.5, DeepSeek V4 Flash, Nemotron 3 Ultra, etc.). Generación de imágenes funcionando con `krea/krea-2-medium-turbo`. Fix de error 401 (strip de prefijo `opencode/`), fix de visualización de modelos en WebUI, `auth models` muestra todos los proveedores. Sección VPS agregada (referral Vultr). 7 READMEs actualizados, 53 docs estilizados. Ver [CHANGELOG.md](CHANGELOG.md).

2026-04-05 🎉 **Integración de Gestión de Contexto + Fix Token Overflow**: Integrada interface ContextManager (legacy + seahorse), motor SQLite Seahorse (~6,800 líneas), verificación de presupuesto pre-build y system prompt de 3 niveles (Mínimo/Compacto/Completo). Error 402 de WebUI + openrouter-free solucionado (21K → ~300 tokens). Todas las plantillas de onboard y auth logins ahora incluyen `context_manager: "seahorse"`. Parches upstream adaptados. Guía de proveedores free publicada. Ver [CHANGELOG.md](CHANGELOG.md).
...
2026-03-27 🎉 **WebUI Launcher totalmente operativo**: `picoclaw-agents-launcher` ahora funciona end-to-end — botón Start Gateway, chat WebSocket via PicoChannel, contenido de skills nativas en página Skills, y todas las secciones validadas. Ejecutar con `picoclaw-agents-launcher` o `picoclaw-agents-launcher -public` para acceso de red.

2026-03-27 🎉 **Pipeline de 3 binarios**: GoReleaser ahora produce los tres binarios — `picoclaw-agents` (CLI), `picoclaw-agents-launcher` (WebUI), y `picoclaw-agents-launcher-tui` (TUI) — coincidiendo con la estructura de releases del proyecto original. Trigger con `./scripts/create-release.sh`.

2026-03-26 🎉 **Documentación MCP Builder**: Documentación completa del MCP Builder Agent en inglés y español con referencia API, casos de uso y ejemplos. Ver [docs/MCP_BUILDER_AGENT.md](docs/MCP_BUILDER_AGENT.md) y [docs/MCP_BUILDER_AGENT.es.md](docs/MCP_BUILDER_AGENT.es.md).

2026-03-26 🎉 **Comandos Sandbox & Codegen**: Añadidos `sandbox init/status` para workspaces aislados y `util codegen` para generación de boilerplate Go. Ver [CHANGELOG.md](CHANGELOG.md) para detalles completos.

2026-03-26 🎉 **Auth Token Monitor**: Añadidos comandos `auth tokens` y `auth monitor` para seguimiento de expiración de tokens OAuth. Ver [CHANGELOG.md](CHANGELOG.md).

2026-03-26 🎉 **Config Validator & Secret Masking**: Añadido comando `config validate` para validación de schema y enmascaramiento de secretos en el wizard de onboard. Ver [CHANGELOG.md](CHANGELOG.md).

2026-03-26 🎉 **Doctor Command**: Añadido comando `doctor` para diagnóstico de entorno incluyendo detección WSL y checks de seguridad. Ver [CHANGELOG.md](CHANGELOG.md).

2026-03-12 🎉 **Soporte Antigravity & Estabilidad**: Soporte completo Google Antigravity OAuth con sanitización de schema, fix de deadlock TokenBudget, mejoras de rehidratación de sesión, nuevo comando `picoclaw-agents clean`, y patrones de deny reforzados. Ver [CHANGELOG.md](CHANGELOG.md) para detalles completos.

2026-03-03 🎉 **Arquitectura de Skills Nativas**: Introducidas skills nativas compiladas directamente en el binario (`pkg/skills/queue_batch.go`), eliminando dependencias de archivos `.md` externos. Seguridad, rendimiento y type safety mejorados. Ver [docs/QUEUE_BATCH.en.md](docs/QUEUE_BATCH.en.md).

2026-03-02 🎉 **Fast-path & Global Tracker**: Añadidos Slash Commands instantáneos (`/bundle_approve`, `/status`, etc.) para interacción de latencia cero. Unificado `ImageGenTracker` en todos los agentes para consistencia perfecta de estado multi-agente. Ver [docs/queue_batch.md](docs/queue_batch.md).

2026-03-01 🎉 **Generación de Imágenes AI & Community Manager**: Añadida generación de imágenes nativa (Gemini/Ideogram), workflows script-to-image, menús interactivos post-generación, y agente community manager para generar posts de redes sociales automáticamente. Ver [docs/IMAGE_GEN_util.md](docs/IMAGE_GEN_util.md) para ejemplos completos.

2026-03-01 🎉 **Integraciones Externas (Binance, Social Media, Notion)**: Añadidas herramientas nativas para trading de criptomonedas (Binance futures & spot), publicación en redes sociales (Facebook & X/Twitter), y gestión de conocimiento (Notion). Configurar via `config.json` o variables de entorno. Ver [SOCIAL_MEDIA.md](SOCIAL_MEDIA.md) y [docs/NOTION_util.md](docs/NOTION_util.md) para guías de configuración.

2026-03-01 🎉 **Native Skills Sentinel**: Añadida capa de seguridad interna (`skills_sentinel.go`) que proporciona protección en tiempo real basada en patrones contra inyección de prompts y filtraciones del sistema.
2026-03-01 🎉 **Endurecimiento de Seguridad y Estabilidad**: Manejo robusto de cierre de message bus, procesamiento resilente de fondo de WeCom App, y validación reforzada de inicialización para la herramienta shell.
2026-03-01 🎉 **Fail-Close Security**: Política de seguridad robusta. La herramienta de ejecución de comandos ahora realiza validación estricta de patrones de deny durante la inicialización.

2026-02-27 🎉 **Recuperación de Desastres & Task Locks**: Implementados Task Locks atómicos para prevenir colisiones de agentes, "Boot Rehydration" para recuperación de caídas abruptas, y Context Compactor (elevando el límite de forma segura a 32K tokens) para erradicar explosiones de contexto en tareas largas de código.
