...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. Unlike the standard Google AI Studio API, Antigravity leverages the credits and quotas included in **Google One AI Premium** and **Workspace Gemini** plans.
...
PicoClaw supports **3 providers** with built-in authentication:
...
The credential includes a `refresh_token` used to obtain new `access_token`s when the current one expires. The system handles refresh in **3 layers**:

1. **Proactive daemon** (`pkg/auth/refresh_daemon.go`): A background goroutine checks every 20 minutes if the token expires within 30 minutes, and renews it preemptively.
2. **Reactive refresh** (`antigravity_provider.go`): Each agent call checks if the token is about to expire (`NeedsRefresh`) **or already expired** (`IsExpired`). If a `refresh_token` exists, it attempts renewal before failing.
3. **Full scopes** (`oauth.go`): The refresh request now uses the same scopes as the initial login (`cloud-platform`, `cclog`, etc.), preventing 403 errors after refresh.

### Token Expiration

- Google's `access_token` lasts **~1 hour** (3599 seconds). This is fixed by Google's server and not configurable.
- The `refresh_token` lasts months/indefinitely, unless revoked.
- Manual re-authentication is only needed if the `refresh_token` was revoked (password change, manual revocation, 6+ months of inactivity).
...
**Cause:** Access token has expired and auto-refresh failed.
...
**Cause:** The Gemini API is not enabled in your Google Cloud project.

**Solution:**
1. Go to [Google Cloud Console](https://console.cloud.google.com)
2. Select your project
3. Navigate to "APIs & Services" > "Library"
4. Search for "Gemini for Google Cloud" and enable it

### ❌ "Project not found"

**Cause:** The project ID could not be fetched or is invalid.

**Solution:**
1. Ensure you have a valid Google Cloud project
...
**Cause:** OAuth authentication may not have completed successfully.
...
**Cause:** Running on a headless server or WSL2.

**Solution:** Use device code flow:
```bash
# Note: Device code flow is currently only supported for OpenAI
# For Antigravity, manually copy the URL shown in terminal to your browser
```

Or manually copy the authorization URL from the terminal and open it in your browser.

---

## See Also

- [`ANTIGRAVITY_USAGE.md`](./ANTIGRAVITY_USAGE.md) - Usage guide and examples
- [`ANTIGRAVITY_AUTH.es.md`](./ANTIGRAVITY_AUTH.es.md) - Spanish version of this guide
- [Google Cloud Console](https://console.cloud.google.com) - Manage your project and quotas
