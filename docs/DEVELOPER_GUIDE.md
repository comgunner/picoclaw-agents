...
touch pkg/skills/my_new_skill.go
```

**Step 2: Implement Skill Interface**

```go
// pkg/skills/my_new_skill.go
package skills
...
func NewMyNewSkill(workspace string) *MyNewSkill {
    return &MyNewSkill{workspace: workspace}
}

func (s *MyNewSkill) Name() string {
    return "my_new_skill"
}

func (s *MyNewSkill) Description() string {
    return "Description of what this skill does"
}

func (s *MyNewSkill) GetInstructions() string {
    return myNewSkillInstructions
}

func (s *MyNewSkill) GetAntiPatterns() string {
    return myNewSkillAntiPatterns
}

func (s *MyNewSkill) GetExamples() string {
    return myNewSkillExamples
}

func (s *MyNewSkill) BuildSkillContext() string {
    var parts []string
    
    parts = append(parts, "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    parts = append(parts, "🚀 NATIVE SKILL: "+s.Name())
    parts = append(parts, "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    parts = append(parts, "")
    parts = append(parts, "**PURPOSE:** "+s.Description())
    parts = append(parts, "")
...
func (s *MyNewSkill) BuildSummary() string {
    return `<skill name="` + s.Name() + `" type="native">
  <purpose>` + s.Description() + `</purpose>
</skill>`
}

// Documentation constants
const myNewSkillInstructions = `## WHEN TO USE
...
const myNewSkillAntiPatterns = `## ANTI-PATTERNS
...
const myNewSkillExamples = `## EXAMPLES
...
func (sl *SkillsLoader) listNativeSkills() []SkillInfo {
    return []SkillInfo{
        {
            Name:        "queue_batch",
            Description: "Delegate heavy tasks to background queue",
            Source:      "builtin",
        },
        {
            Name:        "my_new_skill",
...
func (sl *SkillsLoader) LoadNativeSkills() []string {
    var contexts []string
    
    // Load existing skills
    contexts = append(contexts, sl.LoadNativeQueueBatchSkill())
    
    // Load new skill
    contexts = append(contexts, sl.LoadNativeMyNewSkill())
    
    return contexts
}

func (sl *SkillsLoader) LoadNativeMyNewSkill() string {
    skill := NewMyNewSkill(sl.workspace)
...
func (cb *ContextBuilder) BuildContext() string {
    ...
    // Load native skills
    nativeSkills := cb.loader.LoadNativeSkills()
    parts = append(parts, nativeSkills...)
...
```go
// pkg/skills/my_new_skill_test.go
package skills
...
func TestMyNewSkill_BuildSkillContext(t *testing.T) {
    skill := NewMyNewSkill("/tmp/workspace")
    context := skill.BuildSkillContext()
    
    assert.Contains(t, context, "NATIVE SKILL: my_new_skill")
    assert.Contains(t, context, "PURPOSE:")
}

func TestMyNewSkill_BuildSummary(t *testing.T) {
    skill := NewMyNewSkill("/tmp/workspace")
    summary := skill.BuildSummary()
    
    assert.Contains(t, summary, `<skill name="my_new_skill"`)
}
```
